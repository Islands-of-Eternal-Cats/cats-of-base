...
// Объект-штамп в руке (§12.162): -1 = игрок держит тайл или ластик, а не
// объект. Держится рядом с `maskTile`, но отдельно — маска у объекта отвечает
// на другой вопрос: не «можно ли сюда тайл», а «встанет ли отсюда весь штамп».
let maskStruct = -1;
// Четверть оборота в руке. Едет вместе с инструментом: поворот меняет и рамку,
// и ответ ворот, значит маску надо пересчитать.
let maskRot = 0;
...
function loop() {
  ...
  if (sim) {
    ...
    const mask =
      maskStruct >= 0
        ? sim.structure_buildable(maskStruct, ...maskRect, maskRot)
        : maskTile >= 0
          ? sim.buildable(maskTile, ...maskRect)
          : null;
    postMessage({
      type: "snapshot",
      snap: sim.snapshot(),
...
onmessage = (e) => {
  ...
  if (m.type === "setSpeed") {
    ...
    // Тайл и объект взаимно исключают друг друга: в руке одно.
    maskStruct = m.structure ?? -1;
    maskRot = m.rot ?? 0;
  ...
  } else if (m.type === "place" && sim) {
    // Объект ставится **одним кликом**, без рамки: размер у него свой, и
    // тянуть его мышью значило бы дать игроку выбрать то, что выбрано в
    // рулсете. Решение атомарно — ворота считает фасад (§12.160).
    sim.place_structure(m.structure, m.x, m.y, m.rot);
  ...
  } else if (m.type === "setBin" && sim) {
    // Что держать в ящике (§12.195). Ворота одни — у клетки должен быть ярус, —
    // и считает их фасад: второй экземпляр в JS однажды покажет живой значок
    // там, где ядро откажет.
    sim.set_bin(m.x, m.y, m.item, m.on);
    ...
    // `site` — куда идти (§12.198). У обычного заказа место своё, и вид его не
    // называет: `-1` значит «не назван», и ядро берёт место самого заказа. У
    // зачистки это и есть решение игрока — какой очаг тушить первым.
    sim.launch_node(m.mission, m.x, m.y, m.site ?? -1);
