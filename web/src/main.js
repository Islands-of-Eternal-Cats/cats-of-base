...
/// Полные характеристики кота словами, без единой шкалы (§12.53): подсказка на
/// портрете в штабе. Набирая отряд, игрок решает не «кто сильнее в поле» — на
/// это отвечает строка рядом, — а «кого нельзя отсюда забирать»: учёного с
/// «Наукой», носильщика с перком. Ответ на это стоит в «Личном деле», то есть
/// в другом окне и по коту за раз, а решение принимают здесь и списком.
///
/// Полосок тут нет намеренно: подсказка живёт под курсором и читается за
/// секунду, а доля до следующего уровня на выбор в отряд не влияет — влияет
/// сам уровень. Порядок тот же, что в панели кота: врождённое и перки до
/// навыков, потому что объясняют их пределы (§12.42).
function catFacts(e) {
  const lines = [];
  const born = (meta.stats ?? [])
    .map((st, i) => `${st.label || st.id} ${e.stats?.[i] ?? 0}`)
    .concat((e.perks ?? []).map((id) => perkLabel(id)));
  if (born.length) lines.push(born.join(" · "));
  // Нулевой навык не пишем вовсе — даже с накопленным опытом: подсказку читают
  // за секунду, а «Ремесло 0» на выбор в отряд не влияет ничем и топит в себе
  // те два домена, ради которых кота с вылазки и снимают.
  const skills = (meta.skills ?? [])
    .map((def, i) => {
      const s = e.skills?.[i];
      if (!s || s.level === 0) return "";
      const capped = s.cap > 0 && s.level >= s.cap;
      // Упёршийся навык говорит об этом словом: «Наука 3 (предел)» отвечает на
      // «подучить ли его ещё», не открывая «Личное дело».
      return `${def.label || def.id} ${s.level}${capped ? " (предел)" : ""}`;
    })
    .filter(Boolean);
  lines.push(skills.length ? skills.join(" · ") : "навыков пока нет");
  return lines.join("\n");
}
...
onPanelClick(raidWinEl, ".raid-go", (b) => {
  ...
  sendAction({
    ...
    x: Number(b.dataset.x),
    y: Number(b.dataset.y),
    // Куда идти (§12.198): у зачистки кнопка своя на каждый очаг, у обычного
    // заказа участка нет вовсе — ядро возьмёт место самого заказа.
    site: b.dataset.site == null ? -1 : Number(b.dataset.site),
  });
});
...
// Справочник грузится один раз за страницу и **не блокирует старт** (§12.204):
// тексты принадлежат игре, а не партии, и партия обязана подниматься, даже
// если файлы не доехали, — тогда значков «i» просто нет. Ждать их здесь значило
...
let raidUi = null;
// Заказы, чьи карточки показывались за этот визит в штаб (§12.243): гасить
// новость на закрытии можно только про них.
const raidSeenDefs = new Set();
...
let shops = 0;
// Сколько на базе складских клеток (§12.215): различает «склада нет» и «лежит
// не там» — см. `payHint`.
let stores = 0;
...
let raids = [];
// Состояние карты внешнего мира: что на каком участке растёт (§12.198).
// Подпись, координаты и соседей вид берёт из палитры `meta.sites`, а отсюда —
// только очаг и его прогноз. Считает прогноз **ядро** тем же выражением, каким
// шагает система роста: посчитай его здесь — и карта однажды пообещает одно, а
// очаг сядет в другое место (инвариант 14).
let sites = [];
...
worker.onmessage = (e) => {
  ...
  if (m.type === "ready") {
    ...
    // ...и выбранный на карте участок: палитра участков у нового мира своя, и
    // индекс от прошлого указал бы на чужое место (§12.198). Сами узлы графа
    // пересоберутся вместе с каркасом штаба.
    mapPick = -1;
    buildToolbar();
    layout();
    drawMap(m.map);
...
// Повороты почти нулевые: у вытянутого глифа (колба) наклон в 12° читается как
// другой размер, а не как «навалено». Стопка — сдвиг, а не разброс.
const STACK_LAYOUT = [
  [-0.09, 0.08, -0.05],
  [0.09, 0.0, 0.04],
  [0.0, -0.09, -0.02],
...
function drawScrap(list) {
  ...
  for (const s of list ?? []) {
    ...
    // Несколько типов на клетке: глиф **того же размера**, что и в одиночку, —
    // ужатие делало колбу на смешанной полке вдвое меньше колбы на чистой, и
    // это читалось как разный объём. Типы ложатся со сдвигом внахлёст, сеткой
    // 2×2, не вылезая из клетки; больше четырёх — по кругу в те же места.
    const GRID = [
      [-0.14, -0.12],
      [0.14, -0.12],
      [-0.14, 0.12],
    ...
    const cell =
      m === 1 ? [0, 0] : m === 2 ? [slot ? 0.14 : -0.14, 0] : GRID[slot % 4];
  ...
  scrapEl.innerHTML =
    (pinned ||
      `<span class="stock off" data-tip="Ресурсы в шапку не выбраны: ` +
        `отметьте нужные звёздочкой в окне «Ресурсы»">` +
    ...
    //
    // ⚠️ **Незнакомой стороны в шапке нет вовсе** (§12.214): «0» у фракции, о
    // которой игра ещё не сказала ни слова, — это цифра без вопроса, к которому
    // она ответ, и стоит она в самом дорогом месте экрана. Знакомство считает
    // ядро (`factions_met`) и считает **монотонно**: репутация умеет вернуться
    // в ноль, а знакомство закрываться не умеет (§12.131).
    (meta.factions ?? [])
      .map((f, i) => {
        if (!(snap.factions_met ?? [])[i]) return "";
    ...
    //
    // ⚠️ **Пока счёт не заводили, его в шапке нет** (§12.214), и меряется это
    // журналом `Earned` (`money_seen`), а не самим счётом: вход у денег
    // единственный — продажа, — а расход обнуляет счёт легко, и «¤0» у
    // потратившего всё читалось бы как «денег в этой игре не бывает».
    (snap.money_seen
...
function tileRoles(def) {
  ...
  // Лаборатории нет по тому же доводу, что парты (§12.233): у неё своя
  // карточка, и тема в её заголовке уже говорит, что это за клетка.
...
function renderCellPanel(snap) {
  ...
  const parts = [
    // Значок «i» в справочник (§12.204). Панель клетки отвечает на «что тут
    // сейчас», а «что это вообще за комната» — вопрос к статье, и это
    // **дверь, а не решение**, поэтому §12.80 к ней не применим: открыть окно
    // ничего не стоит и отменяется закрытием (прецедент — «Назначить» у ящика,
    // §12.195). §12.101 цел: модал открывает не клик по карте, а кнопка в уже
    // открытой панели — жест второй и осознанный.
    //
    // ⚠️ **Стоит он перед именем, а не после** (§12.218): значок один и тот же
    // на все шесть мест, и в конце строки его место плавало — за именем, за
    // ценой, за координатами, — то есть глазами его каждый раз искали заново.
    `<div class="cat-name">${def ? wikiMark(`tile:${def.id}`) : ""}` +
      `${esc(name)} <span class="cell-at">${x}, ${y}</span></div>`,
  ];
  ...
  if (bp) {
    ...
    parts.push(
      '<div class="cat-skill">' +
        ...
        // Постройку ставят на чистый пол (§12.237): пока уборка не унесла
        // кучи, строитель не придёт, и без этой строки площадка с материалом
        // на месте молчала бы о том, чего ждёт (§12.53). Число — из ядра.
        (bp.clutter > 0
          ? `<div class="cat-sub">ждёт уборки: здесь лежит ${bp.clutter}</div>`
          : "") +
...
function crewList(snap, x, y) {
  ...
  //
  // То же верно и для **чужого** ушедшего отряда (§12.247): группа «В других
  // отрядах» пустела ровно на время его вылазки и возвращалась с ней, то есть
  // мигала, — и читалось это как «соседний отряд распустили». Ушедший чужой
  // кот стоит строкой с «на вылазке», не нажимается (`off` по `away`) и
  // отвечает на «где второй отряд» там же, где свои отвечают на «где мой».
  // Свободных ушедших не бывает: без отряда с базы не уходят, а пленный без
  // приписки — вопрос своей панели.
  //
  ...
  const cats = (snap.entities ?? []).filter(
    (e) => !e.away || here(e) || squadNo(e) > 0,
  );
  ...
  const rows = cats.map((e) => {
    ...
    // Наука, мастерская и парта — долгая работа, которую вылазка оборвёт на
    // уходе: тема и заказ останутся без рук, учёба встанет. Это не отказ (кот
    // пойдёт), а цена решения (§12.231), поэтому цвет свой, не красный. Во всех
    // группах (§12.234): свободного и чужого кота берут в отряд одним кликом,
    // и оборвёт это ту же работу — выбор «брать ли» стоит как раз перед ними.
    const toil =
      !e.away &&
      !hurt &&
    ...
    const pick =
      ...
      ">" +
      portraitHtml(e.sprite, "crew-face", catFacts(e)) +
      ...
      (note
        ? `<i class="crew-note${idle ? " bad" : toil ? " toil" : ""}">${esc(note)}</i>`
        : "") +
      "</span></button>";
  ...
  for (let band = 0; band < heads.length; band++) {
    ...
    // Исключение — «В других отрядах» при двух и более гаражах (§12.247):
    // группа стоит всегда, даже пустой, — иначе она появляется и исчезает
    // вслед за чужим набором, и список под составом прыгает.
    if (!group.length && !(band === 3 && nodes.length > 1)) continue;
...
function cellWork(snap, x, y, def) {
  if (!def) return [];
  const out = [];
  // Лаборатория говорит карточкой (`labCell`, §12.233), а не строкой.
  if (def.gate) {
...
// Те же три стадии словом **про участок** — подпись под картой штаба
// (§12.245): «сюда идёт» у отряда на дороге домой врало.
const SITE_PHASE = {
  travel: "сюда идёт",
  work: "здесь работает",
};
...
function craftTip(g, pct) {
  ...
  lines.push(
    g.auto
      ? "по порогу — снимается в окне «Ресурсы»"
...
function labCell(snap, x, y) {
  ...
  if (!r) {
    return [
      '<div class="cat-sub">лаборатория свободна — темы берут в окне «Наука»</div>',
    ];
  }
  const pct = r.total > 0 ? Math.round((r.progress / r.total) * 100) : 0;
  const owed = r.owed ?? [];
  // Имя учёного стоит в заголовке, как ученик у парты; строка остаётся только
  // там, где ей есть что сказать. Ждёт образец — не то же, что ждёт
  // исполнителя (§12.133): первое чинится вылазкой, второе временем.
  const state = owed.length
    ...
    : r.unit
      ...
      : r.home === false
        ...
        : "ждёт исполнителя";
  const who = r.unit && !owed.length ? ` · ${esc(r.unit)}` : "";
  return [
    '<div class="cat-skill">' +
      `<div class="cat-row"><span>${esc(topicLabel(r.def))}${who}</span><b>${pct}%</b></div>` +
      `<div class="bar"><i style="width:${pct}%"></i></div>` +
      (state ? `<div class="cat-sub">${state}</div>` : "") +
      // Плата образцами не возвращается (§12.132), а завезённый образец
...
function craftCell(snap, x, y) {
  ...
  if (!c) {
    return [
      '<div class="cat-sub">станок свободен — заказы в окне «Ресурсы»</div>',
    ];
  }
  ...
  const foot = c.auto
    ? '<div class="cat-sub">по порогу — снимается в окне «Ресурсы»</div>'
...
function renderNotePanel(list, tick = 0) {
  ...
  const rows = notes.map((n) => {
    ...
    // ⚠️ **Безусловное событие «успели» не говорит** (§12.198). `succeeded` — это
    // «база была готова», и у события без `requires` оно истинно всегда: до
    // §12.198 таких событий не было вовсе, а «Разлив у старых шахт» именно
    // такой — он ничего не требует и ничего не даёт, он **сеет очаг**. Зелёное
    // «успели» над строкой «в шахтах осядет биозаражение» читается как успех
    // ровно там, где случилась беда. Прошедшее безусловное событие просто
    // случилось, и это всё, что о нём можно сказать честно.
    //
    // Различать их можно только у прошедшего: до раскрытия `requires` пуст у
    // всех (§4.6, туман предзнания), а к своему тику событие раскрыто всегда.
    const unconditional = !(n.requires ?? []).length;
    const when = n.done
      ? unconditional
        ? '<span class="cat-sub">случилось</span>'
        : n.succeeded
...
// клика: у двери она остаётся под накрывшим её окном (§12.101). Со §12.212
// обёртка эта висит только на дверях — кнопки «Курсор» больше нет, — и она же
// **единственный** мышиный выход из режима разметки, кроме заголовка самой
// «Постройки». Заголовки разделов
...
// Нажатие мыши где угодно вне карты и вне раздела «Постройка» кладёт
// инструмент из рук (§12.236). Липкость режима (§12.62) — про карту и палитру
// под рукой; щелчок по панели, шапке или окну значит «я тут закончил», а мышь,
// вернувшаяся на карту, продолжала бы строить. Раздел при этом не сворачивается:
// палитра остаётся открытой, выбрать следующий тайл — один клик.
// Слушаем на документе в фазе захвата, до обработчиков самих узлов: панели
// перерисовываются кадром, и цель может исчезнуть раньше, чем событие всплывёт.
document.addEventListener(
  "mousedown",
  (e) => {
    if (mode === "cursor") return;
    const t = e.target;
    if (!(t instanceof Node) || app.canvas.contains(t)) return;
    const build = sections.find((s) => s.title === "Постройка");
    if (build && (build.head.contains(t) || build.body.contains(t))) return;
    selectCursor();
  },
  true,
);
...
function buildToolbar() {
  ...
  // ⚠️ **Кнопки «Курсор» здесь больше нет** (§12.212). Она называла состояние
  // «ничего не размечаю», то есть предлагала **перестать** — а из липкого
  // режима (§12.62) и без неё ведут три выхода, и все три уже написаны:
  // Escape первым же нажатием, свёрнутый щелчком заголовок «Постройки» и любая
  // другая кнопка верхнего уровня (§12.194, `folds` → `openOnly` → инструмент
  // из рук). Четвёртая дорога к тому же — не инструмент, а строка в столбце,
  // где место дорогое, и стояла она первой. `selectCursor` жив и зовётся всеми
  // тремя.
  //
  // Дверь — вне разделов: это не режим, а окно (§12.100). Раздел из одной
  // кнопки был бы лишним кликом — окно на базе одно, выбирать в списке нечего.
  //
  // ⚠️ **Зовётся она «Ресурсы», а не «Складом»** (§12.217): «Склад» — это тайл,
  // одна клетка с ёмкостью, и одно слово на клетку и на всё имущество базы
  // читалось как «открыть ту постройку». Окно же отвечает на «что у базы есть»
  // целиком, считая пол, лапы и бронь (§12.53), — то есть ровно не про склад.
  const ware = mkTool(
    '<span class="sw sw-scrap"></span><span>Ресурсы</span>',
  ...
  meta.palette.forEach((p, i) => {
    ...
    // Значок «i» — **первым в строке** (§12.218) и внутри самой кнопки, как у
    // темы и кандидата: палитра это столбик кнопок, и вынести сноску наружу
    // значило бы перестроить вёрстку ради неё. Место у него тут единственное
    // возможное: справа стоит цена набором с `margin-left: auto` (§12.225), и
    // всё, что встанет за именем, отожмёт последнюю фишку за край.
    const b = mkTool(
      `${wikiMark(`tile:${p.id}`)}${glyph}<span>${p.label || p.id}</span>${cost}`,
      (e) => {
        // Клик по «i» всплывает до самой кнопки, поэтому отсекается первым
        // (§12.204): без этого справочник брал бы постройку в руки и гасил её
        // новость (§12.224) — то есть отвечал бы на «что это» действием.
        ...
        if (b.classList.contains("off")) return;
        // ⚠️ **Кнопка гасит свою новость поштучно** (§12.224). До этого метку
        // палитры снимал только выход из раздела (`openOnly`) — то есть все
        // постройки разом и ни одной в отдельности, — и взятая в руки
        // «Лаборатория» оставалась обведённой, а её тикер висел внизу, пока
        // игрок её строил. Клик по кнопке — свидетельство сильнее, чем уход из
        // раздела: список игрок не просто увидел, он **взял отсюда инструмент**.
        // Гашение на открытии (§12.120) сюда не относится — раздел уже раскрыт,
        // и это действие, а не дорога посмотреть.
        readNews("tile", [i]);
        selectBuild(i, b);
      },
    );
  ...
  // ⚠️ **Своей двери у карты нет** (§12.199): она живёт средней колонкой в
  // штабе, потому что заказ и есть место (§12.198), и выбор заказа с выбором
  // места — один жест. Дверь «Карта» тут была ровно один заход и вела в окно,
  // которое отвечало «где горит» и отсылало тушить в другое окно.
  const files = mkTool(
  ...
  // «Справочник» (§12.204). Дверь не гаснет и не прячется: лор партии не
  ...
  // неё нет: к вещам ведут значки «i» рядом с самими вещами.
  const wiki = mkTool(
...
//
// ⚠️ **Совет при этом различает «склада нет» и «лежит не там»** (§12.215): со
// §12.215 партия начинается без единой складской клетки, и «уберите на склад»
// звало бы убрать туда, чего нет, — то есть было бы отказом без выхода. Это та
...
function payHint(cost) {
  ...
  for (const [id, need] of entries) {
    ...
    const tail = st.loose
      ? `, ещё ${st.loose} валяется — ` +
        (stores > 0 ? "уберите на склад" : "постройте склад, и коты уберут")
      : "";
  ...
  topicButtons.forEach((b, i) => {
    ...
    if (b._run) {
      ...
      if (run) {
        ...
        if (b._run.state.textContent !== state)
          b._run.state.textContent = state;
      }
    }
    const ready =
      !t.known &&
...
// Заказ, которого сейчас нет вовсе (§12.40, §12.199): известность не дошла
// или у заказа нет цели — за своим без пленных, зачистка без очагов. Признак
// `rescue` тут не нужен: вопрос один, считает его ядро (`RaidSnap::possible`).
//
// Это «заказа не существует», а не «пока нельзя»: по нему заказ пропадает и
// из колонки штаба, и из подписи участка на карте.
function goneRaid(i) {
  const gates = raids[i];
  if (!gates?.unlocked) return true;
  return !(gates.possible ?? true);
}

// Что прячется из **колонки штаба** (§12.242): пропавший заказ и заказ, чьё
// место заражено. Заражённый до §12.242 стоял карточкой с причиной словом
// (§12.79), но карточка ничего не предлагала: ни кнопки, ни состава, — а
// колонка от неё росла. Ответ на «куда делся заказ» — сама карта: участок
// горит, а в колонке по нему (§12.243) стоит одна карточка зачистки
// (`cardForSite`) — она же дорога к возвращению.
function hiddenRaid(i) {
  if (goneRaid(i)) return true;
  return !(raids[i]?.reachable ?? true);
}

// Заказчик не доверяет, а заказ игрок ещё **не видел открытым** (§12.246):
// известность есть, репутации нет, `met` пуст. Такой заказ карточкой не стоит —
// вместо него строка на фракцию внизу колонки — и в счёт на кружке карты не
// идёт: счёт обещает карточку по клику. Заказ, открытый хоть раз (`met` —
// монотонная память ядра, `RaidsMet`), при упавшей репутации **остаётся
// карточкой** с причиной словом и без живой кнопки: это «потерял», а не «не
// дорос», и игрок про него помнит. Ворота те же, что у строки причины
// (`RaidSnap::welcome`), второго экземпляра нет.
function untrustedRaid(i) {
  const g = raids[i];
  return !!g?.unlocked && !g?.welcome && !g?.met;
}
// Что встанет карточкой в колонку по клику на участок — одно выражение на
// счёт в кружке, гашение новостей по клику и саму колонку (§12.245).
function shownRaid(i) {
  return !hiddenRaid(i) && !untrustedRaid(i);
}
...
//
// Подпись, координаты и соседей вид берёт из палитры (`meta.sites`), состояние
// очага — из снимка (`snap.sites`). Прогноз («станет ступенью 3 через 240»,
// «расползётся на Старые шахты») считает **ядро** и везёт тегом `step`: второй
// экземпляр этой арифметики в JS однажды покажет стрелку не туда, куда сядет
// очаг, — тот же довод, что у `MissionSnap::phase` (§12.168).

// Индекс участка по имени из рулсета. Имена ходят только в палитре: у заказа в
// `meta.missions[i].site` лежит строка, а всё остальное считается индексом.
function siteIndexOf(id) {
  if (!id) return -1;
  return (meta?.sites ?? []).findIndex((s) => s.id === id);
}

function siteLabel(i) {
  const s = (meta?.sites ?? [])[i];
  return s ? s.label || s.id : "";
}

function blightLabel(i) {
  const b = (meta?.blights ?? [])[i];
  return b ? b.label || b.id : "";
}

// Что на участке — одной строкой: порода и ступень. Ступень называется всегда,
// в том числе первая: «Биозаражение» без числа не отвечает на «насколько
// поздно», а это и есть цена промедления (§12.198).
function blightWords(i) {
  const st = sites[i];
  if (!st || st.blight == null) return "";
  return `${blightLabel(st.blight)} · ступень ${st.stage} из ${st.top}`;
}

// Причина отказа у заказа, стоящего на заражённом участке.
function siteWhy(id) {
  const i = siteIndexOf(id);
  const what = blightWords(i);
  return `${siteLabel(i)}${what ? ` — ${what}` : ""}`;
}

// Прогноз участка словом. Тег считает ядро, здесь только называем: «grow» —
// поднимется ступенью, «spread» — пойдёт к соседу (и к какому именно),
// «held» — расти и ползти некуда.
function stepWords(i) {
  const st = sites[i];
  if (!st || !st.step) return "";
  if (st.step === "held") return "дальше некуда — расти и ползти некуда";
  if (st.step === "spread") {
    return `через ${st.step_in} расползётся на «${siteLabel(st.step_to)}»`;
  }
...
function raidGate(i, node) {
  ...
  // Цели у заказа нет вовсе: за своим — все дома, у зачистки — нигде не горит
  // (§12.199). Обе ветки защитные: по этому же признаку `goneRaid` карточку
  // прячет, и до причины дело не доходит. Держим их по тому же доводу, что и
  // «нужна известность»: ворота у обеих функций одни, и молчащая карточка при
  // расхождении хуже лишней строки.
  const nobody = !(gates?.possible ?? true);
  const nothingWhy = def.cleanses
    ? "нигде не горит — тушить нечего"
  ...
  //
  // ⚠️ **Заказ занимает и живое правило соседа** (§12.183): для «свободен ли
  // заказ» идущая вылазка соседа и его же круг по тому же заказу — один ответ,
  // и строка «эту вылазку уже ведёт: Отряд N» стоит в карточке одна на обе
  // причины. Живая кнопка под ней предлагала бы второй отряд туда же, а игрок
  // потом видел бы, что на заказ ходят по очереди двое, и читал бы это как
  // случайность.
  //
  ...
  // не мешает. Расхождение здесь безопасно и однонаправленно — вид **строже**
  // фасада, то есть обещанного и отклонённого не бывает (ср. §12.111, где
  // запрещено обратное: показать живым то, что фасад отклонит).
  const ruled = nodes.some((o) => o !== node && o.auto === i && o.auto_on);
  const taken = running.has(i) || ruled;
  // Ворота места (§12.198): участок заказа заражён — или у зачистки не осталось
  // ни одного очага. Считает их ядро (`RaidSnap::reachable`), здесь только
  // называем словом: у обычного заказа это «где горит, туда не ходят», у
  // зачистки — «тушить нечего». Список целей тоже из ядра — тем же выражением,
  // каким заявка выберет цель (инвариант 14).
  // Место заказа заражено (§12.198). Только про заказ **со своим местом**:
  // у зачистки места нет, а «есть ли что тушить» спрашивает `possible` выше.
  const reachable = gates?.reachable ?? true;
  const targets = gates?.targets ?? [];
  const cleans = !!def.cleanses;
  const blightWhy = reachable
    ? null
    : `участок заражён: ${esc(siteWhy(def.site))}`;
  ...
  // вовсе — ветки «нужна известность» и «участок заражён» здесь держим
  // защитными: ворота у обеих функций одни (`RaidSnap`), и молчащая карточка
  // при расхождении хуже лишней строки.
  //
  ...
  const title = nogate
    ...
    : distrust
      ? `${hint} · ${distrust}`
      : // ⚠️ **Заражение называется раньше занятости** — по тому же доводу, что и
        // репутация: «отряд занят» пройдёт само, а очаг сам не уйдёт, он только
        // растёт. Игрок, дождавшийся отряда ради этой карточки, узнал бы про
        // закрытый участок последним (§12.116).
        blightWhy
        ? `${hint} · ${blightWhy}`
        : busyWhy
          ? // ⚠️ **Прогноза здесь нет вовсе** (§12.182): пока узел занят, `hint`
            ...
            busyWhy
          : taken
            ? // Одним словом на обе причины (§12.183): «ведёт» — это ответ на
              // «свободен ли заказ», а кто именно ведёт, названо строкой выше
              // номером отряда.
              "эту вылазку уже ведёт другой отряд — он назван строкой выше"
            : !known
  ...
  return {
    ...
    distrust: known && !!distrust,
    // Куда этот заказ может пойти (§12.198). У обычного пусто: место у него
    // одно и названо в палитре, выбирать нечего.
    targets,
    cleans,
...
// --- карта в штабе (§12.198, §12.199) --------------------------------------
//
// **Карта живёт в штабе, а не своим окном.** После §12.198 заказ и есть место:
// выбор заказа и выбор места — один жест, и разносить его по двум модалам
// нечем оправдать (тот же довод, которым §12.166 удалила раздел «Вылазки» из
// тулбара). Своё окно «Карта» отвечало на «где горит» и тут же говорило «а
// тушить иди в штаб» — то есть было экраном, с которого решение принять нельзя.
//
// Но карта **не съедает колонку заказов**, и держат её две вещи, обе не про
// вёрстку:
//   * цена решения обязана быть словами и до клика (§12.80, §12.53), а в кружок
//     не влезает ни «сила 2 против сложности 9», ни «дорога 90 + работа 300 /
//     2 кота», ни причина отказа;
//   * **заказ без места** — «За своим» не привязан к участку намеренно, иначе
//     плен стал бы необратим (§12.40), и пина у него нет и быть не может.
//
// Поэтому кружок **подсвечивает и выбирает**, а не фильтрует: все заказы
// остаются в колонке, клик прокручивает к нужному. Фильтр прятал бы закрытые
// заказы, а §12.79 держит их видимыми ровно потому, что это цель, к которой
// игрок идёт.
// Выбранный участок; **`-1` значит «выбирает игра»**, а не «не выбрано».
//
// Разница принципиальная (§12.199). Цель зачистки нужна игроку под рукой в тот
// самый миг, когда где-то загорелось, — а загорается оно, пока он смотрит в
// другую сторону. Пока игрок карту не трогал, выбор **следует за самым
// запущенным очагом** (тем же порядком, каким ядро выбирает цель правилу
// автовылазки). Первый же клик по кружку записывает число, и дальше карта
// слушается игрока, даже если он отметил чистый участок: решение принял он.
//
// Латать это флагом «выбирал ли руками» не пришлось ровно потому, что «авто» —
// это отдельное состояние выбора, а не его отсутствие.
let mapPick = -1;
// Радиус кружка и шаг сетки в единицах SVG. Числа условные: `at` у участка —
// подача, а не механика (§12.198), и расстояние между кружками ни на что не
// влияет.
const SITE_R = 22;
const SITE_STEP = 78;

// Какой участок выбран прямо сейчас: названный игроком или, пока он не называл,
// самый запущенный очаг. Одно выражение на карту, на кнопку заказа и на
// прокрутку — иначе кружок подсветится один, а отряд уйдёт на другой.
function pickedSite() {
  if (mapPick >= 0) return mapPick;
  const worst = (sites ?? [])
    .map((st, i) => [st.blight == null ? -1 : st.stage, i])
    .sort((a, b) => b[0] - a[0])[0];
  return worst && worst[0] > 0 ? worst[1] : 0;
}

// Какая карточка отвечает за этот участок. У горящего это **зачистка**: там
// стоит кнопка, ради которой в карту и смотрят; у чистого — его собственный
// заказ. `null` — карточки нет вовсе (участок без заказов и без очага).
function cardForSite(i) {
  const st = sites[i] ?? {};
  const list = meta?.missions ?? [];
  if (st.blight != null) {
    const cure = list.findIndex(
      (m) => m.cleanses && (raids[list.indexOf(m)]?.targets ?? []).includes(i),
    );
    if (cure >= 0) return cure;
  }
  const own = list.findIndex(
    (m, k) => m.site && siteIndexOf(m.site) === i && !goneRaid(k),
  );
  return own >= 0 ? own : null;
}

// Прокрутить колонку заказов к карточке выбранного участка. Прокрутка законна
// ровно потому, что колонка строится один раз и живёт узлами (§12.118): её
// `scrollTop` никто не отматывает.
function scrollToSiteCard(i) {
  const def = cardForSite(i);
  if (def == null || !raidUi) return;
  const el = raidUi.cardEls.get(String(def));
  if (el && !el.hidden) el.scrollIntoView({ block: "nearest" });
}

// Цель зачистки, выбранная игроком на карте (§12.199): выбранный участок, если
// на нём и правда горит нужная порода. Иначе `null` — и кнопка гаснет с
// причиной словом, а не отправляет отряд туда, куда игрок не показывал.
//
// Список целей считает **ядро** (`RaidSnap::targets`) — тем же выражением,
// каким заявка выберет цель, — здесь только сверяемся с выбором (инвариант 14).
function pickedTarget(def) {
  const targets = raids[def]?.targets ?? [];
  const at = pickedSite();
  return targets.includes(at) ? at : null;
}

// Про этот ли участок заказ (§12.243). У заказа с местом — его `site`; у
// зачистки — любой участок из списка целей, который считает **ядро**
// (`RaidSnap::targets`, инвариант 14): «Выжечь очаг» стоит там, где горит, и
// нигде больше. Заказ без места (за своим) не про участок вовсе — `placeless`.
function raidAtSite(i, site) {
  const m = (meta?.missions ?? [])[i];
  if (!m) return false;
  if (m.site) return siteIndexOf(m.site) === site;
  if (m.cleanses) return (raids[i]?.targets ?? []).includes(site);
  return false;
}
function raidPlaceless(i) {
  const m = (meta?.missions ?? [])[i];
  return !!m && !m.site && !m.cleanses;
}

// Куда на карте ведёт заказ: своё место, у зачистки — первая цель. `-1` — никуда.
function siteOfRaid(i) {
  const m = (meta?.missions ?? [])[i];
  if (!m) return -1;
  if (m.site) return siteIndexOf(m.site);
  if (m.cleanses) return (raids[i]?.targets ?? [])[0] ?? -1;
  return -1;
}

// Перекрасить карту под нынешний снимок. Узлы уже стоят (`buildSiteGraph`),
// меняются только заливка, ступень, метка отряда и подпись под графом.
function syncRaidMap() {
  if (!raidUi?.marks?.length) return;
  // Выбор мира из прошлой партии сюда не доедет: `ready` сбрасывает `mapPick`
  // в «авто», а число за пределами палитры значило бы чужое место.
  if (mapPick >= raidUi.marks.length) mapPick = -1;
  const pick = pickedSite();
  // Кто куда идёт: цель вылазки заморожена в заявке (§12.22), и по ней участок
  // метится именем отряда. Своих на карте не видно — половина ответа на «где
  // мы» пропадает.
  // Стадию называет ядро (`MissionSnap::phase`, §12.168): «сюда идёт» про
  // отряд на дороге домой — неправда, которую игрок видит по срокам. А
  // возвращающийся на карте не метится вовсе: участок он уже покинул, и
  // «где отряд» отвечает карточка идущей вылазки, а не кружок.
  const going = new Map();
  missionsOut.forEach((m) => {
    if (m.site >= 0 && m.phase !== "back") {
      const n = nodes.findIndex((o) => o.x === m.x && o.y === m.y);
      going.set(m.site, { who: `Отряд ${n + 1}`, phase: m.phase });
    }
  });

  // Сколько заказов на участке — числом **в центре кружка** (§12.245): колонка
  // показывает заказы только выбранного участка, и без счёта на карте игрок
  // на старте не знал, куда вообще смотреть — все кружки одинаково пусты.
  // Центр делит со ступенью очага, и они не спорят: у горящего участка свои
  // заказы скрыты (§12.242), там число — ступень, у чистого — счёт. Считаются
  // те же заказы, что встанут в колонку по клику (`hiddenRaid` + `raidAtSite`);
  // «9+» — потолок. Непрочитанный заказ красит число акцентом — прежней
  // отдельной точки нет. Заказы без места (за своим) на карте не считаются
  // вовсе: они стоят в колонке при любом участке.
  const fresh = newlyOpen("raid");
  const defs = (meta?.missions ?? []).map((_, k) => k);
  raidUi.marks.forEach((g, i) => {
    const st = sites[i] ?? {};
    const lit = st.blight != null;
    g.classList.toggle("on", i === pick);
    g.classList.toggle("hot", lit);
    g.classList.toggle("mine", going.has(i));
    const here = defs.filter((k) => shownRaid(k) && raidAtSite(k, i));
    g.classList.toggle(
      "fresh",
      here.some((k) => fresh.has(k)),
    );
    const n = here.length > 9 ? "9+" : here.length ? String(here.length) : "";
    // Ступень числом прямо в кружке: «насколько поздно» — это и есть цена
    // промедления, и читать её игрок должен не наводя мышь (§12.53). У
    // чистого участка на том же месте — счёт заказов.
    const stage = g.querySelector(".map-stage");
    const shown = lit ? String(st.stage) : n;
    if (stage.textContent !== shown) stage.textContent = shown;
    const squad = g.querySelector(".map-squad");
    const who = going.get(i)?.who ?? "";
    if (squad.textContent !== who) squad.textContent = who;
    // Чем гуще заливка, тем ближе предел. Доля, а не три цвета: ступеней у
    // породы бывает сколько угодно, и словарь цветов под них не заведёшь.
    g.querySelector(".map-ring").style.opacity = lit
      ? String(0.25 + 0.6 * (st.stage / Math.max(1, st.top)))
      : "";
  });

  // Подпись под графом — вся из ядра: что растёт, докуда доросло и что будет
  // дальше (§12.198).
  const i = pick;
  const st = sites[i] ?? {};
  const lines = [`<div class="map-h">${esc(siteLabel(i))}</div>`];
  if (st.blight != null) {
    lines.push(`<div class="bad">${esc(blightWords(i))}</div>`);
    const next = stepWords(i);
    if (next) lines.push(`<div class="warn">${esc(next)}</div>`);
    // Закрытые очагом заказы здесь не перечисляются (§12.243): с колонкой
    // по участку их место занимает карточка зачистки, а имя заказа часто
    // совпадает с именем участка и читалось дублем заголовка.
  } else {
    // Заказы чистого участка здесь не перечисляются: с §12.243 они и есть
    // колонка справа, и список рядом отвечал бы на тот же вопрос второй раз.
    lines.push('<div class="good">чисто</div>');
  }
  const trip = going.get(i);
  if (trip) {
    const word = SITE_PHASE[trip.phase] ?? SITE_PHASE.travel;
    lines.push(`<div class="warn">${word} ${esc(trip.who)}</div>`);
  }
  setHtml(raidUi.note, lines.join(""));
}
...
function newsText(n) {
  ...
  if (n.kind === "recipe") {
    // Закрытий у рецепта не бывает (§12.18), но ветку пишем: молчаливое «i» на
  ...
  if (n.kind === "rule") {
    ...
    // (§12.18); ветку пишем по доводу рецепта и постройки: молчаливое «i» на
    // экране объяснить было бы нечем.
...
function openNewsTarget(kind, def) {
  ...
  if (kind === "raid") {
    // Колонка штаба режется участком (§12.243): новость ведёт на участок
    // заказа, иначе карточка, за которой пришли, спрятана.
    const at = siteOfRaid(def);
    if (at >= 0) mapPick = at;
    openRaidWindow();
...
// Ключ статьи справочника для новости (§12.204). Виды ленты и виды статей —
...
// словарь. Нет палитры (или записи в ней) — нет и значка «i».
...
function renderNews(snap) {
  ...
  for (const n of want) {
    ...
    if (!row) {
      ...
      // Значок «i» стоит **до** «×» и ведёт в справочник, а сама строка
      // по-прежнему ведёт к действию (§12.204): новость это предложение
...
// Закрыта ли ещё сама постройка, в которой работает механика за дверью
// (§12.211, §12.213). Ворота берём **у тайла** (`tiles_open`, §12.210), а не
// переписываем его условие: их считает ядро одним выражением, и второй
// экземпляр разошёлся бы с палитрой на первой же правке контента.
//
// Одно выражение на обе двери намеренно: правило у них общее, а не два похожих
// частных случая, — «Наука» стоит за лабораторией, «Покупка» за торговым
// постом, и обе прячутся ровно до той минуты, пока игра о своей постройке
// молчит.
function doorTileClosed(snap, has) {
...
function syncDoors(snap) {
  ...
  if (sciDoor) {
    ...
    // есть чего ждать.
    // ⚠️ **И прячем, пока закрыта сама лаборатория** (§12.213, расширяет
    // §12.211 на вторую дверь). §12.151 оставляет тусклую дверь ради второго
    // ответа — «открывшееся окно говорит, чего ждать», — но на старте окно
    // «Науки» пусто по-настоящему: единственная тема ждёт находки и скрыта
    // (§12.143), витрины (§12.137) тоже нет. Второго ответа за дверью не
    // существует, а первый («сходите на вылазку») дублирует единственное
    // доступное дело партии. Образец приезжает — открывается и лаборатория в
    ...
    // Идущая тема — то же зрелище, что витрина: взять новую нельзя, а смотреть,
    // докуда дошла начатая, можно и нужно (правая колонка окна). Гашение
    // говорило бы «за дверью нечего делать» про окно, где идёт работа.
    const running = (snap.research ?? []).length > 0;
    const blocked = !open.length || !staffed || !room;
    sciDoor.classList.toggle("off", !teasing && !running && blocked);
    ...
    const why = !open.length
      ...
      : // Лаборатория спрашивается **раньше** учёного (§12.230) — тем же порядком, что
        // у кнопки темы в окне: без неё звать за парту значит отправлять
        // кота учиться ради комнаты, которой нет, а следующий шаг после
        // первой вылазки — именно стройка (новость о лаборатории приходит
        // тем же тиком, что образец).
        !anyLab
    ...
    liveTitle(
      sciDoor,
      running && blocked && open.length
        ? `Идут исследования — загляните, докуда дошли. Новую не начать: ${why}`
        : why,
    );
  }
  if (hireDoor) {
  ...
  //
  // ⚠️ **Но пока сама постройка закрыта, двери нет вовсе** (§12.211). Это
  // третий случай рядом с «механики нет в рулсете» и «список закрыт навсегда»
  // (§12.151, §12.156), и довод у него свой: причина отказа зовёт построить
  // «Торговый пост», а с §12.210 его до первой находки нет и в палитре. Дверь
  // требовала вещь, о существовании которой игра сама молчит, — ровно то, из-за
  // чего §12.143 прячет тему, ждущую находки. Слово «пока» тут никуда не
  // делось, просто произносить его рано: постройка открывается новостью
  // (`NewsKind::Tile`), и дверь приезжает вместе с ней.
  //
  // Ворота берём **у самого тайла** — общим `doorTileClosed`, тем же, что и у
  // «Науки» (§12.213).
  if (buyDoor) {
    const unbuildable = doorTileClosed(snap, (p) => p.trade);
    buyDoor.hidden = unbuildable;
    // Окно могло стоять открытым — закрываем сами, как «Найм» у §12.156:
    // список без входа и выхода наружу висел бы поверх карты.
    if (unbuildable && buyWinOpen) closeBuyWindow();
    const noPost = !posts;
  ...
  (meta.research ?? []).forEach((r, i) => {
    ...
    const b = mkTool(
      '<span class="topic-main">' +
        // Значок «i» живёт **внутри** кнопки темы: список окна — столбик
        // кнопок, и вынести его наружу значило бы перестроить вёрстку ради
        // сноски. Стоит он **первым в строке имени** (§12.218) — там же, где у
        // клетки, новости, кандидата, заказа и предмета. Отсюда и отсечка в
        // обработчике ниже (§12.204): клик по «i» всплывает до самой кнопки, и
        // без неё справочник запускал бы исследование.
        wikiMark(`topic:${r.id}`) +
...
function buildHireWindow() {
  ...
  (meta.recruits ?? []).forEach((r, i) => {
    const b = mkTool(
      ...
      // Значок справочника отсекаем первым (§12.204): он внутри кнопки, и клик
...
//
// ⚠️ **Оно возвращает туда, откуда открыто.** Игрок жмёт «i» у строки склада,
...
//
// ⚠️ **Значок «i» появляется только там, где статья есть** (`wikiMark`).
...
// Значок «i» рядом с вещью. Не «?»: тот уже стоит в тулбаре у подсказки
// режима (`.hint-toggle`), и два разных вопросительных знака в одном окне
// читаются одной кнопкой. «i» же говорит «справка», а не «спроси».
// ⚠️ Ключ у кнопки обязателен (`data-key`): панели
...
function wikiMark(key, extra = "") {
  ...
  return (
    ...
    `data-tip="Что это такое">i</button>`
  );
}

// Живые факты о записи — **перекличкой палитр**, а не строкой в тексте
// (§12.204, идиома `opensOf`). Цена стеллажа, вход рецепта, опасность заказа
...
/// они были бы недостижимы, потому что значок «i» стоит только у вещей.
...
// Значок «i» ловится **на документе**: он стоит в десятке разных панелей и
...
// пока окно ещё открыто (§12.204): у «Ящика», «Личного дела» и штаба адрес
...
function buildStockWindow() {
  ...
  (meta.items ?? []).forEach((it, item) => {
    ...
    // Значок «i» в справочник (§12.204) — **перед** глифом и именем (§12.218):
    // он про саму вещь, а не про её запас, и место у него одно на все шесть
    // строк проекта. Строится один раз вместе со строкой: окно складское
    // (§12.118), и статья у предмета за партию не меняется.
    name.insertAdjacentHTML("afterbegin", wikiMark(`item:${it.id}`));
...
function syncStockWindow() {
  ...
  const warns = [];
  // ⚠️ **Третье состояние у торговли завелось со §12.223** — до неё их было
  // два, потому что поста не закрывала никакая технология. Теперь закрывает
  // («Торговая сеть»), и правило про порядок проверок работает здесь дословно
  // так же, как у мастерской строкой ниже: пока постройка не открыта, красная
  // строка звала бы строить кнопку, которой нет в палитре (§12.126, §12.211).
  // Ворота берём **у самого тайла**, общим `doorTileClosed`: второй экземпляр
  // этого правила в JS разошёлся бы с ядром (инвариант 14).
  const tradeKnown = !doorTileClosed(lastSnap ?? {}, (p) => p.trade);
  if (!canTrade && tradeKnown)
    warns.push("Торговля недоступна: нет «Торгового поста»");
...
function closeRaidWindow() {
  // Гасим на закрытии: колонка заказов в штабе видна **всегда**, даже когда
...
function buildRaidWindow() {
  raidWinEl.innerHTML =
    ...
    "</div>" +
    // Карта — средней колонкой, между составом и заказами (§12.199): она и есть
    // то, о чём заказ, и стоять ей полагается там, где на неё смотрят, решая.
    '<div class="raidwin-col raidwin-map" hidden>' +
    '<div class="raidwin-h">Карта</div>' +
  ...
  raidUi = {
    ...
    rule: q(".raid-rules"),
    cards: q(".raid-cards"),
    mapCol: q(".raidwin-map"),
    note: q(".map-note"),
    // Узлы участков — по индексу. Строятся **вместе с каркасом**, то есть один
    // раз на открытие окна и на смену вкладки: граф от вкладки не зависит, но
    // пересобрать его в этот момент законно ровно потому же, почему законна
    // пересборка всего каркаса (§12.118).
    marks: buildSiteGraph(q(".map-holder")),
  ...
  raidUi.mapCol.hidden = !raidUi.marks.length;
}

// Граф участков в SVG (§12.198, §12.199): кружок на участок, линия на соседство.
//
// Рисуется **один раз** и дальше только перекрашивается (`syncRaidMap`):
// пересобирай его каждым кадром, и клик по кружку умирал бы так же, как умирали
// кнопки в перерисовываемых панелях (§12.84).
//
// Координаты `at` — подача, а не механика (§12.198): расстояние между кружками
// ни на что не влияет, дорога до места — свойство заказа (`travel`).
function buildSiteGraph(holder) {
  const all = meta?.sites ?? [];
  if (!holder || !all.length) return [];
  const xs = all.map((s) => (s.at ?? [0, 0])[0]);
  const ys = all.map((s) => (s.at ?? [0, 0])[1]);
  const w = (Math.max(...xs) - Math.min(...xs)) * SITE_STEP + SITE_R * 6;
  const h = (Math.max(...ys) - Math.min(...ys)) * SITE_STEP + SITE_R * 6;
  const px = (i) =>
    ((all[i].at ?? [0, 0])[0] - Math.min(...xs)) * SITE_STEP + SITE_R * 3;
  const py = (i) =>
    ((all[i].at ?? [0, 0])[1] - Math.min(...ys)) * SITE_STEP + SITE_R * 3;

  // Рёбра — по списку соседей **из снимка** (`sites[i].links`): он уже
  // симметричный, потому что разворачивает связь ядро (§12.198). Считать
  // обратные связи здесь значило бы завести второй экземпляр правила, по
  // которому ползёт очаг. Пару рисуем один раз — `a < b`.
  const edges = [];
  (sites ?? []).forEach((st, a) =>
    (st.links ?? []).forEach((b) => {
  ...
  holder.innerHTML =
    ...
    edges
      .map(
        ([a, b]) =>
          `<line class="map-link" x1="${px(a)}" y1="${py(a)}" x2="${px(b)}" y2="${py(b)}"/>`,
      )
      .join("") +
    all
      .map(
        (s, i) =>
          `<g class="map-site" data-site="${i}" transform="translate(${px(i)},${py(i)})">` +
          `<circle class="map-ring" r="${SITE_R}"/>` +
          `<text class="map-stage" y="6"></text>` +
          `<text class="map-name" y="${SITE_R + 18}">${esc(s.label || s.id)}</text>` +
          `<text class="map-squad" y="${SITE_R + 32}"></text>` +
          "</g>",
      )
      .join("") +
    "</svg>";
  const marks = [...holder.querySelectorAll(".map-site")];
  // Клик по кружку — **выбор цели, а не отправка** (§12.75, §12.199): он
  // подсвечивает участок, прокручивает к его заказу и становится целью для
  // зачистки. Сама отправка остаётся кнопкой в карточке, где рядом написана
  // цена. `mousedown`, а не `click`, — по общему правилу проекта (§12.84).
  // Клик по кружку заодно **гасит новости его заказов** (§12.245, дословно
  // §12.224 у кнопки палитры): игрок явно пошёл смотреть этот участок, и
  // акцентное число на нём после клика говорило бы «не читано» про то, что
  // стоит в колонке справа. Закрытие окна остаётся вторыми воротами — для
  // участка, на который игрок не кликал, но чью карточку видел.
  marks.forEach((g, i) => {
    g.onmousedown = () => {
      mapPick = i;
      readNews(
        ...
        (meta?.missions ?? [])
          .map((_, k) => k)
          .filter((k) => shownRaid(k) && raidAtSite(k, i)),
      );
      renderRaidWindow();
      scrollToSiteCard(i);
    };
...
// Строка на фракцию про невиданные заказы, закрытые её недоверием (§12.246): сколько их и
// какой порог. Порог — первая недостающая фракция из `needs`, та же, что
// называет `trustGap`; у заказа с двумя порогами вторая ждёт своей очереди.
function trustLines(node) {
  const factions = meta?.factions ?? [];
  const byFaction = new Map();
  (meta?.missions ?? []).forEach((def, i) => {
    if (hiddenRaid(i) || !untrustedRaid(i) || node.auto === i) return;
    const needs = def.needs;
    const entries =
      needs instanceof Map ? [...needs.entries()] : Object.entries(needs ?? {});
    for (const [id, want] of entries) {
      const f = factions.findIndex((x) => x.id === id);
      if (f < 0) continue;
      const have = standing[f] ?? 0;
      if (have < want) {
        const row = byFaction.get(f) ?? { count: 0, want: 0, have };
        row.count += 1;
        row.want = Math.max(row.want, want);
        byFaction.set(f, row);
        break;
      }
    }
  ...
  return [...byFaction.entries()]
    .map(
      ([f, r]) =>
        `<div class="cat-sub warn">${esc(factions[f].label || factions[f].id)}: ` +
        `${plural(r.count, "заказ", "заказа", "заказов")} — нужна репутация ${r.want}, у вас ${r.have}</div>`,
    )
    .join("");
}
...
function renderRaidWindow() {
  ...
  // Карта — **после карточек**: прокрутка к выбранному участку ищет их узлы, и
  // до первой синхронизации их ещё нет.
  syncRaidMap();
  raidWinEl.hidden = false;
}
...
function syncRaidCards(raid, node) {
  ...
  // Колонка — про выбранный на карте участок (§12.243): заказ с другого места
  // и зачистка, которой здесь нечего жечь, прячутся. Остаются заказы без
  // места (за своим) и заказ правила — он про решение игрока, а не про место.
  const pick = pickedSite();
  const here = (i) =>
    node.auto === i || raidPlaceless(i) || raidAtSite(i, pick);
  // Закрытый репутацией заказ карточкой не стоит (§12.246) — если только на
  // него не поставлено правило: правило ждёт ворот, и снять его можно лишь из
  // карточки.
  const open = (meta.missions ?? []).map(
    (_, i) =>
      !hiddenRaid(i) &&
      raid?.def !== i &&
      (!untrustedRaid(i) || node.auto === i),
  );
  const want = (meta.missions ?? []).map((_, i) => [
    String(i),
    open[i] && here(i) ? raidCard(i, node) : "",
    i,
  ]);
  if (want.every(([, html]) => !html)) {
    ...
    // (§12.53). Третья причина — участок: заказы есть, но не здесь, и слово
    // обязано звать к карте, а не к известности.
    want.push([
      "none",
      raid
        ? '<div class="cat-sub">других заказов нет — этот отряд уже в деле</div>'
        : open.some(Boolean)
          ? `<div class="cat-sub">на участке «${esc(siteLabel(pick))}» заказов нет — выбери другой на карте</div>`
          : nothing,
    ]);
  }
  // Что игрок здесь видел, — ради гашения новостей на закрытии (§12.243):
  // спрятанный участком заказ прочитанным не считается.
  for (const [, html, i] of want) if (html && i != null) raidSeenDefs.add(i);
  ...
  // Заказы, закрытые репутацией и не виданные открытыми, — **строкой на
  // фракцию** внизу колонки, без карточек (§12.246): пока репутации нет, ни
  // добыча, ни срок ни в каком решении не участвуют, а нужны три вещи — что
  // работа есть, у кого и насколько не хватает. Виданный остаётся карточкой. Про участок строка не знает намеренно: репутация —
  // про сторону, а не про место, и колонка по участку её не режет. Прятать
  // совсем нельзя: репутация падает (§12.43), и заказы исчезали бы молча.
  want.push(["trust", trustLines(node)]);
...
function raidCard(i, node) {
  ...
  // ⚠️ **У заказа на зачистку общего вердикта нет** (§12.198): целей несколько,
  // сложность у них разная, и одно число обещало бы исход самого запущенного
  // очага на кнопке свежего. Вердикт стоит у каждой цели — там же, где кнопка,
  // которую по нему и нажимают. Тот же довод, по которому §12.182 убрал прогноз
  // у занятого узла: второй ответ хуже, чем ни одного.
  // ⚠️ **Цель зачистки выбирают на карте** (§12.199): у такого заказа она одна —
  // тот участок, который игрок отметил кружком, — и прогноз считается **по
  // ней**. До §12.199 целей было столько, сколько очагов, и они стояли
  // столбиком кнопок: на горящей карте это шесть строк, съедавших карточку, а
  // общего вердикта у них не было вовсе. Кружок и есть тот же выбор, только
  // сделанный там, где видно, что рядом с чем.
  const aim = g.cleans ? pickedTarget(i) : null;
  // Исход по этой цели считает **ядро** (`NodeSnap::aims`) — тем же `outcome`,
  // каким он посчитается на возвращении (инвариант 14).
  const hit =
    aim == null
      ? null
      : (node.aims ?? []).find((a) => a.def === i && a.site === aim);
  const share = hit ? hit.share : g.share;
  const failed = hit ? hit.failed : g.failed;
  const danger = hit ? hit.danger : g.danger;
    ...
    : empty
      ...
      : failed
        ...
        : share >= 100
          ...
          : `<span class="warn">добыча ${share} %</span>`;
  ...
  // против чего (§12.71).
  //
  // У зачистки «было» не пишем: её сложность сложена из своей и ступени очага,
  // и «было 2» под числом 9 назвало бы виновником проводника вместо очага.
  // Вместо этого называем ступень — она и есть причина, и она же цена
  // промедления (§12.198).
  const cut =
    !g.busy && !hit && g.danger !== g.base
      ? ` (было ${g.base} — ведёт ${esc(g.guide)})`
      : hit
        ? ` · очаг ступени ${sites[aim]?.stage ?? 1}`
        : "";
  rows.push(
    `<div class="raidwin-verdict">${verdict}` +
      `<i>${
        empty || g.busy
          ? `сложность ${danger}`
          : `сила ${node.force ?? 0} против сложности ${danger}${cut}`
      }</i></div>`,
  );
  ...
  // ⚠️ **У зачистки цель называет карта** (§12.199), а не столбик кнопок в
  // карточке. Кнопка одна — как у любого другого заказа, — и на ней написано,
  // куда пойдут: выбор сделан кружком, там, где видно, что рядом с чем и куда
  // пойдёт огонь. До §12.199 кнопок было столько, сколько очагов, и на горящей
  // карте шесть строк съедали карточку, а прогноз у каждой был свой.
  //
  // Выбран чистый участок — кнопка гаснет **со своей причиной**: это не ворота
  // ядра (те считает `raidGate`), а состояние выбора, и §12.53 требует назвать
  // его словом ровно так же.
  const blocked = g.cleans && g.targets.length > 0 && aim == null;
  const why = blocked
    ? "на выбранном участке очага нет — отметьте горящий на карте"
    : g.title;
  const live = g.ready && !blocked;
  // У заказа, который игрок видел открытым, а заказчик закрыл (§12.246),
  // кнопки нет: «Нельзя» под красной строкой с числами — отказ там, где строка
  // уже назвала цель. Прочие закрытые кнопку держат — их причина живёт в её
  // подсказке (§12.181).
  const go = g.distrust
    ? ""
    : `<button class="tool raid-go${live ? " on" : " off"}"` +
      (live ? "" : ` data-tip="${esc(why)}"`) +
      ` data-key="go${i}@${at}" data-def="${i}"` +
      (aim == null ? "" : ` data-site="${aim}"`) +
      ` data-x="${node.x}" data-y="${node.y}">` +
      (live
        ? aim == null
          ? "Отправить"
          : // Двоеточие, а не «на Свалку»: склонять имена участков нечем, а
            // именительный после предлога читается как опечатка. Идиома в проекте
            // уже есть — «отряд занят: Свалка» в `busyWhy`.
            `Отправить: ${esc(siteLabel(aim))}`
        : "Нельзя") +
      "</button>";
  ...
  return (
    ...
    // Значок «i» у имени заказа (§12.204): что это за место и чем оно
    // славится — вопрос к справочнику, а «стоит ли идти» отвечает сама
    // карточка числами ниже. Стоит он **перед** именем (§12.218).
    `<div class="raidwin-name">${wikiMark(`raid:${def.id ?? ""}`)}` +
    `${esc(def.label || def.id || "Вылазка")}</div>` +
    rows.join("") +
...
// Своей кнопки у этого состояния нет (§12.212): «ничего не размечаю» — не
// инструмент в ряду прочих, а отсутствие разметки, и рамке вокруг него не на
// чём стоять. Гасим все кнопки разом (`activate(null)`) — погашенный тулбар и
// есть весь ответ. Зовут отсюда: Escape, `openOnly` на выходе из
// «Постройки», `folds` у дверей и нажатие мыши вне карты и палитры (§12.236).
