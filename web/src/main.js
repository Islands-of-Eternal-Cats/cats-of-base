...
// колонка от неё росла. Ответ на «куда делся заказ» переехал на карту: подпись
// участка перечисляет закрытые заказы через `goneRaid`, а не через это
// выражение, иначе перечислять было бы нечего. Дорога к возвращению там же —
// у горящего участка карточка зачистки (`cardForSite`).
...
function syncRaidMap() {
  ...
  raidUi.marks.forEach((g, i) => {
    ...
    // промедления, и читать её игрок должен не наводя мышь (§12.53).
    g.querySelector(".map-stage").textContent = lit ? String(st.stage) : "";
    const squad = g.querySelector(".map-squad");
    const who = going.get(i) ?? "";
  ...
  // дальше (§12.198). Заказы участка называем здесь же: заражённый участок
  // закрывает их, и «что я теряю» — половина ответа на «идти ли туда».
  const i = pick;
  const st = sites[i] ?? {};
  const here = (meta.missions ?? [])
    .map((m, k) => [m, k])
    .filter(([m, k]) => m.site && siteIndexOf(m.site) === i && !goneRaid(k))
    .map(([m]) => esc(m.label || m.id));
  ...
  if (st.blight != null) {
    ...
    lines.push(
      here.length
        ? `<div class="cat-sub">заказы отсюда закрыты: ${here.join(" · ")}</div>`
        : '<div class="cat-sub">заказов здесь нет — но соседям есть куда гореть</div>',
    );
  } else {
    lines.push('<div class="good">чисто</div>');
    lines.push(
      here.length
        ? `<div class="cat-sub">заказы: ${here.join(" · ")}</div>`
        : '<div class="cat-sub">заказов здесь нет</div>',
    );
  }
  const who = going.get(i);
  if (who) lines.push(`<div class="warn">сюда идёт ${esc(who)}</div>`);
