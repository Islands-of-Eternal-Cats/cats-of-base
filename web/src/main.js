...
function syncDoors(snap) {
  ...
  //
  // Ворота берём **у самого тайла** (`tiles_open`, §12.210), а не переписываем
  // его условие: их считает ядро одним выражением, и второй экземпляр разошёлся
  // бы с палитрой на первой же правке контента.
  if (buyDoor) {
    const postTile = (meta.palette ?? []).findIndex((p) => p.trade);
    const unbuildable =
      postTile >= 0 && (snap.tiles_open ?? [])[postTile] === false;
