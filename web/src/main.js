...
// Заказ, которого сейчас нет вовсе, из списка **прячется** (§12.40, §12.199).
//
// Ворота двое, и они не про «пока нельзя»: известность (§12.79) и «есть ли у
// заказа цель». Второе накрывает оба заказа без своего места разом — за своим
// без пленных и зачистку без очагов, — и признак `rescue` тут больше не нужен:
// вопрос один, считает его ядро (`RaidSnap::possible`), а видов заказа может
// стать и третий.
//
// ⚠️ Не путать с `reachable`: заражённый участок заказ **не прячет**. Это
// временно и чинится решением игрока, то есть ровно та цель, к которой он идёт,
// — и карточка остаётся с причиной словом (§12.53, §12.79).
...
function raidGate(i, node) {
  ...
  // (§12.199). Обе ветки защитные: по этому же признаку `hiddenRaid` карточку
...
function cardForSite(i) {
  ...
  const own = list.findIndex(
    (m, k) => m.site && siteIndexOf(m.site) === i && !hiddenRaid(k),
  );
...
function syncRaidMap() {
  ...
  const here = (meta.missions ?? [])
    .map((m, k) => [m, k])
    .filter(([m, k]) => m.site && siteIndexOf(m.site) === i && !hiddenRaid(k))
