...
function crewList(snap, x, y) {
  ...
  const rows = cats.map((e) => {
    ...
    // пойдёт), а цена решения (§12.231), поэтому цвет свой, не красный, — и только у
    // своих: у чужого или свободного кота эта работа ничего не стоит отряду.
    const toil =
      mine &&
      !e.away &&
      !hurt &&
    ...
    const tip = hurt
      ...
      : node?.busy
        ...
        : idle
          ...
          : toil
            ? e.job === "research"
              ? "Уйдёт с отрядом — тема останется без него. Исключить: клик"
              : e.job === "study"
                ? "Уйдёт с отрядом — учёба встанет до его возвращения. Исключить: клик"
                : "Уйдёт с отрядом — заказ мастерской останется без него. Исключить: клик"
            : no
...
// есть весь ответ. Зовут отсюда трое: Escape, `openOnly` на выходе из
// «Постройки» и `folds` у дверей.
