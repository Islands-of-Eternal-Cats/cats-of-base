...
function renderCellPanel(snap) {
  ...
  const parts = [
    `<div class="cat-name">${esc(name)} <span class="cell-at">${x}, ${y}</span>` +
      // Значок «?» в справочник (§12.204). Панель клетки отвечает на «что тут
  ...
  (meta.research ?? []).forEach((r, i) => {
    ...
    const b = mkTool(
      '<span class="topic-main">' +
        ...
        // сноски. Отсюда и отсечка в обработчике ниже (§12.204): клик по «?»
...
function buildStockWindow() {
  ...
  (meta.items ?? []).forEach((it, item) => {
    ...
    // Значок «?» в справочник (§12.204) — после имени, до чисел: он про саму
...
function raidCard(i, node) {
  ...
  return (
    ...
    // Значок «?» у имени заказа (§12.204): что это за место и чем оно
