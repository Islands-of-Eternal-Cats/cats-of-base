...
impl Sim {
    ...
    pub(crate) fn raid_gates(&mut self, def: usize) -> RaidGates {
        ...
        // ответ на «есть ли он вообще»: нет очага — нет и заказа.
        ...
        RaidGates {
            ...
            // остаться видимым с причиной словом (§12.79).
            reachable: site_is_clear(self.world.resource::<Blights>(), rule.site),
            targets,
    ...
    /// Изучена ли тема — тем же вопросом, что и `start_research`: у темы и
    /// технологии одно имя, и знает его `Techs`, а не отдельный список
    /// изученных.
    pub(crate) fn topic_is_learned(&mut self, def: usize) -> bool {
        let Some(rule) = self.world.resource::<ResearchRules>().0.get(def).cloned() else {
            return false;
        };
        self.world.resource::<Techs>().knows(&rule.id)
    }

    /// Открыта ли тема для **ленты**: предыдущие технологии на месте, образец,
    /// если он ей нужен, база хоть раз видела (§12.143), **и на карте стоит хотя
    /// бы одна лаборатория** (§12.222).
    ///
    /// Первые двое — те же, что у строки окна и у двери (§12.143): считай их
    /// лента иначе, и новость «новая тема» пришла бы нулевым тиком, когда
    /// показывать ещё нечего, — то есть разошлась бы с экраном ровно так, как
    /// запрещает инвариант 14.
    ///
    /// Третьи — **только у ленты, и это не расхождение, а разные вопросы**
    /// (§12.222). Окно отвечает «чего ждать» и потому обязано показать тему
    /// раньше постройки — иначе пропадёт единственный ответ на «зачем мне
    /// лаборатория». Лента же **предлагает действовать** (§12.120), и её строка
    /// прямо говорит «лаборатория готова к теме»: без единой лаборатории это
    /// неправда, а вместе с пятью другими новостями первой вылазки — ещё и шум.
    /// Ворота эти умеют закрываться (лабораторию сносят), и это правда о мире:
    /// «тема закрылась» лента уже умеет сказать.
    ///
    ...
    /// известности (§12.79) и **есть ли у заказа цель** — кого спасать у
    /// вылазки за своим (§12.40), что тушить у зачистки (§12.199).
    ///
    /// Второе выражено одним полем (`possible`) намеренно: вопрос у обоих один
    /// — «существует ли сейчас то, за чем идут», — и оба заказа при отрицании
    /// прячутся. Разными их делает только слово в причине, и то живёт в виде.
    ///
    ...
    fn note_news(&mut self) {
        ...
        for kind in NewsKind::ALL {
            for def in 0..counts[kind.index()] {
                ...
                // **Изученная — не тема вовсе**, по тому же доводу: ей нечего
                // ни открыться, ни закрыться. Ворота лаборатории (§12.222)
                // умеют закрываться, и без этого среза снесённая после
                // последней темы лаборатория объявляла «тема закрылась» по
                // каждой уже пройденной — столбик из двух десятков тикеров
                // про то, чего у игрока никто не отнял.
                if kind == NewsKind::Topic && self.topic_is_learned(def) {
                    continue;
                }
                let open = match kind {
                    NewsKind::Raid => {
    ...
    fn tech_allows(&mut self, tile: i16) -> bool {
        ...
        match rules.after_of(tile) {
            ...
            // не забываются технологии (§12.18) и находки. Снос лаборатории
            // ради детали иначе уносил бы парту вместе с новостью о ней.
            Some(prev) => self.tile_was_built(prev),
            None => true,
        }
    }

    /// Стоял ли на карте хоть раз такой тайл (§12.210, §12.220).
    ///
    /// Не то же, что `GoalTest::Tile`: цель меряет мир **сейчас** (её можно
    /// потерять сносом), ворота палитры — память мира о случившемся.
    fn tile_was_built(&self, tile: i16) -> bool {
        self.world.resource::<BaseMap>().was_erected(tile)
    }
...
impl Sim {
    ...
    pub fn launch_node(&mut self, def: usize, x: i32, y: i32, site: i32) -> bool {
        ...
        // **Заказ назначается отряду, а не готовым лапам** (§12.191, сужает
        // §12.148). Ворот «все готовы» здесь больше нет: неготовый кот не
        // отменяет решение игрока, а откладывает уход. Отряд записывается в
        // вылазку целиком, спящие и раненые ждут своего часа у себя, и сбор
        // кончается ровно тогда, когда последний из состава встал на шлюз
        // (`run_missions`).
        //
        // §12.148 при этом цел: узел по-прежнему уходит **всем составом или
        // никак**, — изменилось только то, что «никак» стало «пока никак».
        // Правило автовылазки ждёт готовности по-прежнему (`squad_is_fit` в
    ...
    fn launch_at(
        ...
        // один кот, а не отряд.
        // **Кнопка узла берёт весь состав, а не только готовых** (§12.191).
        // Спящий, раненый и вымотанный уход откладывают, а не отменяют: они
        // числятся в отряде и идут к шлюзу, как только встанут (`gather_squad`).
        // Форма `launch` (тест, «этот отряд, любой свободный узел») остаётся
        // прежней (§12.70): состав там назван списком, и ждать в нём некого.
        let crew = if node.is_some() {
            self.node_crew(&units)
        } else {
            self.ready_crew(&units)
        };
        // Кого трогать нельзя: спящего заявка не будит (§12.51), раненого не
        // поднимает (§12.37). Им хватает записи в отряд — маршрут проложит
        // `gather_squad` в тик, когда кот встанет сам.
        let ready: Vec<Entity> = self.ready_crew(&units).iter().map(|&(e, _)| e).collect();
        ...
        // будить их никто не будет. Заснуть по дороге к шлюзу кот тоже может:
        // тогда его ждёт `gather_squad`, который спящего не трогает, потому что
        // истощение из отряда не выводит (§12.23, §12.51).
        for (cat_e, from) in crew {
            self.world.entity_mut(cat_e).insert(Squad(mission_e));
            // Неготовый ждёт у себя: ни задачи с него не снимаем, ни маршрута
            // не даём (§12.191). Разбудить его здесь значило бы отменить
            // «Беречь себя» кнопкой вылазки (§12.51).
            if !ready.contains(&cat_e) {
                continue;
            }
    ...
    fn run_auto_sales(&mut self) {
        ...
        // Ячейки кончились — **продажи** дальше не подаём: `trade` отклонил бы
        // всех подряд, а трейс наполнился бы отказами (§12.45). Разборы при
        // этом идут своим чередом: у них не пост и не сделка, а заказ
        // мастерской, и выход из цикла глушил бы правило, которому поста не
        // надо вовсе, — на этом трофеи копились втрое сверх порога, пока
        // единственная ячейка стояла под продажей лома.
        let mut posts_full = false;
