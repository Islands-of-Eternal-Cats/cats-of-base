...
impl Dest {
    /// Брать только со склада (§12.69, §12.130). Наружу база отдаёт учтённое —
    /// это пост; **станок с §12.130 туда же**: заказ есть трата запаса, и
    /// платить за него надо тем же, чем за найм, науку и аптечку. Полом
...
pub(crate) fn assign_hauls(
    ...
    // ветка в общем списке. Разницы с продажей с §12.130 нет вовсе: к обоим
    // возят **только со складских куч** — заказ есть трата запаса, и платят за
    // него тем же учтённым, чем за найм, науку и аптечку (§12.69).
    ...
    // Забронированное под продажу базе уже не принадлежит (§12.50), и знает
    // это `work_hauls`: подняв груз, он режет недостачу тем же `free_to_spend`.
    // Не срезать её здесь — значит раздать ходку, которую подъём тут же
    // отклонит: кот получает `Haul`, `work_hauls` снимает его тем же тиком, а
    // следующим раздача повторяет всё заново. Это не лишняя ходка (§12.15), а
    ...
    // Учтённое — то, что лежит на клетке с ёмкостью (§12.69). Тратится только
    // оно: ворота продажи и заказа считают склад, и подвоз к посту и к станку
    // обязан брать оттуда же (§12.130). Иначе правило говорит «с пола нельзя»,
    // а коты на глазах носят с пола — и спорят с ним картинкой.
