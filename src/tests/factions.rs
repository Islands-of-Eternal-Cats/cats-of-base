...
/// одинаковые по усилию ветки ведут к разным наградам, и взять обе разом
/// нельзя. Симметричные ветки свелись бы к выбору цвета (§12.43).
///
/// Людей с §12.141 дают **обе** стороны — поровну, и это стережёт
/// `hires_the_same_number_on_either_side`, — а разводит их **темп**: Синдикат
/// платит рано (Гвоздь на известности 20, Заклёпка на 90), Полиция поздно и
/// качеством (Йод на 75, Антенна на 110), но держит ещё и «Логово». Здесь
/// проверяется ближний конец этой шкалы: за работу Полиции заплачено ходоком
/// Синдиката.
#[test]
...
/// **Обе стороны развилки дают поровну людей** (§12.141).
///
/// Кандидат закрыт полом доверия, а доверие знаковое (§12.43): подняв одну
/// сторону, база роняет другую, — значит список кандидатов делится веткой, а не
/// достаётся целиком. Разъедься числа, и одна ветка стала бы выгоднее другой
/// при том же усилии, то есть выбора бы снова не было — ровно то, что
/// `forces_a_side` стережёт у заказов.
///
/// Сверяется **сколько**, а не **кто и когда**: темп у сторон разный намеренно
/// (Синдикат платит рано, Полиция поздно и качеством), и сверять формы значило
/// бы требовать зеркала, которое §12.43 как раз отвергает.
#[test]
fn the_shipped_ruleset_hires_the_same_number_on_either_side() {
    let sim = shipped();
    let needs = sim.recruit_needs();
    assert!(
        !needs.is_empty(),
        "кандидатов в рулсете нет — сверять нечего"
    );

    // Доступно ветке: нейтральные (пол доверия пуст) плюс свои. Кандидат,
    // требующий доверия двух сторон разом, не достанется никому — и не
    // засчитается ни одной, что и есть верный ответ.
    let counts: Vec<usize> = (0..sim.faction_spans().len())
        .map(|f| {
            needs
                .iter()
                .filter(|n| n.iter().all(|&(who, _)| who == f))
                .count()
        })
        .collect();

    assert!(counts.len() > 1, "фракция одна — развилки нет");
    assert!(
        counts.iter().all(|&c| c == counts[0]),
        "стороны дают разное число кандидатов ({counts:?}): при одном усилии одна \
