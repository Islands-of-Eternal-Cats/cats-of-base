...
// --- Очередь целей (§12.209) ------------------------------------------------

/// Цели, до которой очередь не дошла, **не существует**: её не видно и её
/// нельзя взять.
///
/// Второе важнее первого. Условие следующей цели часто выполняется само —
/// «100 лома на складе» набирается раньше, чем игрок дойдёт до первой вылазки, —
/// и цель, которую только прячут, всплывает уже взятой: игрок получает галочку
/// вместо задачи, то есть ровно то, ради отмены чего очередь и заведена.
#[test]
fn a_goal_waiting_its_turn_is_neither_shown_nor_taken() {
    let mut sim = sim_from(&["#####", "#a..#", "#####"]);
    let first = sim.set_goal(GoalTest::Earned(10));
    // Условие следующей выполнено с нулевого тика: коты на базе есть всегда.
    let next = sim.set_goal_after(GoalTest::Cats(1), first);

    sim.tick_n(5);
    assert!(!sim.goal_taken(next), "цель ждёт своей очереди");
    assert!(
        !sim.goal_is_visible(next),
        "и наружу не уходит: прятать её в JS значит объявить её в devtools",
    );

    sim.world.resource_mut::<Earned>().0 = 10;
    sim.tick_n(1);
    assert!(sim.goal_taken(first), "предшественник взят");
    assert!(sim.goal_is_visible(next), "и следующая показалась");
    assert!(
        !sim.goal_taken(next),
        "но не взялась тем же тиком: игрок обязан увидеть задачу, а не галочку",
    );

    sim.tick_n(1);
    assert!(sim.goal_taken(next), "а следующим — взялась");
}

/// Дата условия у ждущей цели считается **с её очереди**, а не с тика, когда
/// мир случайно сошёлся (§12.159): дата у цели, которой ещё нет, — это дата не
/// о ней.
#[test]
fn a_waiting_goal_dates_from_its_turn() {
    let mut sim = sim_from(&["#####", "#a..#", "#####"]);
    let first = sim.set_goal(GoalTest::Earned(10));
    let next = sim.set_goal_after(GoalTest::Cats(1), first);

    sim.tick_n(50); // условие следующей держится всё это время, а её самой нет
    assert!(!sim.goal_taken(next));

    sim.world.resource_mut::<Earned>().0 = 10;
    sim.tick_n(2);
    assert!(sim.goal_taken(next), "очередь дошла");
    let at = sim.goal_at(next).expect("взята");
    assert!(
        at > 50,
        "дата взятия — сегодняшняя, а не задним числом: {at}"
    );
    assert_eq!(
        sim.goal_parts(next),
        vec![at],
        "и дата условия та же: раньше своей очереди цель ничего не держала",
    );
}

/// **Боевой рулсет: очередь идёт вперёд и по обязательным целям.**
///
/// Три ошибки, каждая тихая. Предшественник **ниже по списку** — индекс не
/// разрешится, и цель просто откроется сразу (цикла в ядре не бывает по
/// построению, а вот замысел цепочки развалится молча). Предшественник
/// **скрытый или необязательный** — цель повиснет за тем, чего может не
/// случиться вовсе: скрытая берётся только из провала (§12.40), у
/// необязательной есть срок. И **срок у ждущей цели** — он считается от
/// нулевого тика, а цель до своей очереди не существует, то есть часть срока
/// сгорает до того, как игрок о цели узнал.
#[test]
fn the_shipped_ruleset_chains_goals_forward() {
    let sim = shipped();
    let specs = sim.goal_specs();
    for (def, prev) in sim.goal_order().iter().enumerate() {
        let Some(prev) = *prev else { continue };
        assert!(prev < def, "цель {def} ждёт цель {prev}, стоящую ниже");
        assert_eq!(
            specs[prev].0,
            GoalKind::Required,
            "цель {def} ждёт цель {prev}, которая может не случиться вовсе",
        );
        assert_eq!(
            specs[def].2, None,
            "у цели {def} и очередь, и срок: часть срока сгорит до её появления",
        );
    }
}

/// **Боевой рулсет: на старте открыта ровно одна обязательная цель.**
///
/// В этом вся §12.209: игрок, который ещё не понял ни одной механики, видит
/// одно дело, а не семь. Сторож ловит забытый `after` у новой цели — список
/// снова становится списком, и заметить это можно только пересчитав строки
/// глазами.
#[test]
fn the_shipped_ruleset_opens_one_goal_at_a_time() {
    let mut sim = shipped();
    sim.tick_n(1);
    let open = sim.goals().iter().filter(|g| g.kind == "required").count();
    assert_eq!(open, 1, "обязательных целей на старте {open}, а не одна");
}
