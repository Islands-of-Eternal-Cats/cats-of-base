...
/// Пост в палитре есть, и он **заперт технологией** (§12.223).
///
/// До §12.223 этот сторож требовал обратного — «пост не заперт технологией», —
/// и довод был тот, что без него у лишнего лома нет выхода до науки. Довод
/// остался верным, а вывод из него — нет: выход появился на ступень позже, а не
/// исчез, зато рынок перестал доставаться даром и в одной стопке тикеров с
/// лабораторией (§12.221). «Не позже мастерской» проверяется там, где ступени
/// вообще считаются, — `the_shipped_ruleset_opens_its_market_early` в
/// `tech_tree`.
///
/// Здесь остаётся то, что видно из мира: пост в палитре ровно один и запирает
/// его **тема**, а не находка. Пустые ворота значили бы, что §12.223 отменена
/// правкой контента молча.
#[test]
fn the_shipped_ruleset_has_a_post_to_trade_from() {
    ...
    assert!(
        sim.tile_tech(posts[0]).is_some(),
        "пост открыт находкой, а не наукой: рынок снова достаётся даром (§12.223)",
    );
}
