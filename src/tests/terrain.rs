...
/// Маршрут о завалах знает (§12.249, отменяет §12.35 в этой части): путь
/// ищется кратчайший по времени, и кот обходит кучу, если крюк дешевле, чем
/// пробираться сквозь неё.
#[test]
fn the_path_avoids_clutter_when_the_detour_is_cheaper() {
    // Два пути до (3,1): напрямик через завал (2 шага) или понизу (4 шага).
    let mut sim = sim_from(&["#####", "#a..#", "#...#", "#####"]);
    sim.set_auto_tidy(false);
    sim.put_item(2, 1, 0, 64); // +6 тиков на вход: крюк в два шага дешевле
    sim.tick_n(1); // завал переписывается в карту в начале тика
    ...
    assert_eq!(
        ...
        Some((1, 2)),
        "кот пошёл в обход: сквозь завал дольше, чем вокруг него",
    );
}

/// Но за кучей, к которой послан, кот идёт напрямик: цена стоит на входе в
/// клетку, а от цели никуда не деться (§12.249).
#[test]
fn the_path_still_enters_the_pile_it_is_sent_to() {
    let mut sim = sim_from(&["#####", "#a..#", "#...#", "#####"]);
    sim.set_auto_tidy(false);
    sim.put_item(2, 1, 0, 64);
    sim.tick_n(1);

    assert!(sim.set_target("a", 2, 1), "приказ принят");
    sim.tick_n(1);
    assert_eq!(
        sim.stride_of("a").map(|(to, _, _)| to),
        Some((2, 1)),
        "к самой куче — сразу, без крюка",
    );
}

/// Маленькую кучу кот не обходит: крюк в два шага (4 тика) дороже, чем +1 тик
/// на вход (§12.249). Обход — не рефлекс, а сравнение цен.
#[test]
fn a_small_pile_is_not_worth_a_detour() {
    let mut sim = sim_from(&["#####", "#a..#", "#...#", "#####"]);
    sim.set_auto_tidy(false);
    sim.put_item(2, 1, 0, 4); // +1 тик
    sim.tick_n(1);
    ...
    assert_eq!(
        sim.stride_of("a").map(|(to, _, _)| to),
        Some((2, 1)),
        "напрямик: крюк стоил бы дороже",
    );
}

/// «Ближайший кот» у раздатчиков меряется временем, а не шагами (§12.249):
/// кот в двух шагах через завал приходит позже, чем кот в четырёх по чистому
/// полу, — и чертёж достаётся второму.
#[test]
fn the_nearest_cat_is_the_one_who_arrives_first() {
    // Чертёж на (3,1). `a` в двух шагах, но за завалом на (2,1) и без обхода —
    // коридор в одну клетку; `b` в двух шагах по чистому полу.
    // `a` → (3,1): 8 + 2 = 10 тиков; `b` → (3,1): 2 + 2 = 4 тика.
    ...
    const OTHER: i32 = 2;
    assert!(sim.add_blueprint(3, 1, OTHER), "чертёж поставлен");
    sim.tick_n(1);
    assert!(
        sim.has_assignment("b"),
        "чертёж взял тот, кто придёт раньше — `b`, по чистому полу",
    );
    assert!(!sim.has_assignment("a"), "а не тот, кто ближе по шагам");
}
