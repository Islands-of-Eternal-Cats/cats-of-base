...
/// Боевой рулсет: записка на месте, первое событие приходит подарком, а
/// требующее науки действительно требует её. Ловит контент, где дата раньше,
/// чем база вообще способна что-то сделать, или требование указывает на
/// несуществующую тему.
///
/// Индексы здесь позиционные, как и везде в палитрах (§12.21), и порядок в
/// рулсете **хронологический** — записка читается сверху вниз. С §12.198 между
/// караваном и обвалом встал «Разлив» (тик 2400), который сеет очаг на карте:
/// он не требует ничего и потому в этом стороже не участвует — своё про него
/// проверяет `the_shipped_ruleset_can_answer_its_own_blight`.
#[test]
fn the_shipped_ruleset_has_a_readable_note() {
    let mut sim = Sim::new(include_str!("../../assets/rulesets/core.yaml")).expect("рулсет");
    const CARAVAN: usize = 0;
    const CAVEIN: usize = 2;

    // Первое событие безусловно и приносит: таймлайн должен сперва показать
    // себя подарком, а уже потом требовать.
    assert!(
        sim.note_requires(CARAVAN).is_empty(),
        "караван ничего не требует"
    );
    let before = sim.scrap_total();
    sim.tick_n(2000);
    assert_eq!(sim.happened(CARAVAN), Some(true), "караван пришёл");
    assert!(sim.scrap_total() > before, "и не с пустыми лапами");

    // Обвал требует технологии; ждём, пока детали проступят, — до этого
    // записка молчит о том, чем оно грозит.
    assert!(sim.note_requires(CAVEIN).is_empty(), "срок ещё далеко");
    sim.tick_n(1800);
    assert_eq!(
        sim.note_requires(CAVEIN),
        vec!["materials".to_string()],
        "детали события с требованием уже проступили",
    );
...
fn the_shipped_ruleset_day_is_the_cats_cycle() {
    ...
    //
    // С §12.99 ставок три, и берётся здесь самая дорогая — работа: **сутки это
    // день кота, который весь день работал**. Праздный держится дольше и спит
    // реже — это следствие, а не поломка, ровно как дремота в §12.52.
