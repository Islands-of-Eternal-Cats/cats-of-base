...
/// **Боевой рулсет: лаборатория открывается находкой, парта — темой
/// «Наставничество», а тема — учёным третьего уровня** (§12.210, §12.259).
///
/// Лаборатория ждёт первую непонятную штуку: без неё изучать нечего. Тема
...
/// **Боевой рулсет: первая вылазка открывает ровно одну постройку** (§12.223).
///
...
/// §12.69).
///
/// Остаться должна одна, и та — лаборатория: находка задаёт вопрос «что это»,
/// постройка на него отвечает, и следующее дело названо ровно одно. Ошибка тут
...
fn the_shipped_ruleset_opens_one_building_by_the_first_find() {
    ...
    assert_eq!(
        opened,
        ["lab"],
        "первая вылазка открывает {opened:?} — стопка тикеров снова список, \
