...
/// «Полевой методикой» (§12.260) и находкой (ворота `Seen`, §12.139) и берут материал ногами
...
fn the_shipped_ruleset_researches_its_first_topic() {
    ...
    let methods = sim.topic_index("field_methods").expect("первая тема есть");
    assert!(!sim.start_research(methods), "учёного на базе ещё нет");

    assert!(sim.teach("excellent", "science"));
    sim.tick_n(400); // парта доводит до допуска

    assert!(
        !sim.start_research(lore),
        "образец понимают после «Полевой методики» (§12.260)",
    );
    assert!(sim.start_research(methods), "первая тема образца не просит");
    sim.tick_n(1000);
    assert!(sim.knows_tech("field_methods"), "методика изучена");
...
/// **Боевой рулсет: лаборатория открывается учёным, парта — темой
/// «Наставничество», а тема — учёным третьего уровня** (§12.259, §12.260).
///
/// Лаборатория ждёт того, кто в ней будет работать: находка без учёного её не
/// открывает, а первая тема («Полевая методика») образца не просит. Тема
/// парты — того, кому есть чему учить: «Наука» 3 при потолке парты 2. Ворота
/// монотонные — доросший и потом ушедший (или, как здесь, «разучившийся»)
/// учёный тему не закрывает. Сама парта технологией и открывается.
#[test]
fn the_shipped_ruleset_opens_its_classroom_by_the_first_find() {
    ...
    let desk = sim.tile_index("desk").expect("парта в палитре") as usize;
    let science = sim.skill_index("science").expect("наука");
    let topic = sim.topic_index("tutoring").expect("тема «Наставничество»");

    assert!(!sim.add_blueprint(7, 8, lab), "до находки лаборатории нет");
    assert!(!sim.tile_is_open(desk), "и парты нет");
    let stamp = sim
        .structures
        .iter()
    ...
    sim.sight("sample");
    assert!(
        !sim.add_blueprint(7, 8, lab),
        "находка без учёного лабораторию не открывает"
    );

    sim.set_xp("sp3", science, 200); // первый уровень «Науки»
    sim.tick_n(1);
    assert!(sim.add_blueprint(7, 8, lab), "учёный открыл лабораторию");
    sim.force_tile(7, 8, lab as i16);
    sim.set_tech("materials");
    assert!(!sim.topic_is_open(topic), "без учёного темы нет");

    sim.set_xp("sp3", science, 600);
    sim.tick_n(1);
    assert!(!sim.topic_is_open(topic), "второй уровень — ещё не учитель");

    sim.set_xp("sp3", science, 1400);
    sim.tick_n(1);
    assert!(sim.topic_is_open(topic), "третий уровень открыл тему");
    assert!(
        !sim.tile_is_open(desk),
        "а парту открывает только изученная тема"
    );

    sim.set_xp("sp3", science, 0);
    sim.tick_n(1);
    assert!(
        sim.topic_is_open(topic),
        "открытое не закрывается: учёного не стало, а тема осталась",
    );

    sim.set_tech("tutoring");
    assert!(sim.tile_is_open(desk), "тема открыла парту");
    assert!(
...
fn a_demolished_predecessor_keeps_its_successor_open() {
    ...
    // Собственные ворота парты (§12.259) здесь не про то — снимаем их.
...
/// постройки. Гараж отсюда особенно: вылазка не требует ни одной другой
/// постройки, значит любые ворота по находке игрок откроет сам, чем бы он базу
/// ни застроил. Закрой гараж — и снёсший его ради деталей теряет вылазки, а с
/// ними образцы, науку и всю партию: необратимых исходов в игре нет ни одного
/// (§12.5, §12.37, §12.40).
#[test]
fn the_shipped_ruleset_never_locks_its_way_back() {
    let mut sim = Sim::new(include_str!("../../assets/rulesets/core.yaml")).expect("рулсет");
    for id in ["floor", "garage"] {
        let tile = sim.tile_index(id).expect("тайл в палитре");
        assert!(
            sim.tile_is_open(tile as usize),
            "`{id}` закрыт воротами, а это путь восстановления",
        );
    }
}

/// **Боевой рулсет: склад открыт с нулевого тика** (§12.216).
///
/// Единственная постройка, которую игрок может завести до первой вылазки, — и
/// заводит он её ради того, чтобы посмотреть, как коты сами понесут в неё
/// валяющийся запас. Закрой её находкой (а до §12.216 она такой и была), и
/// роль склада объясняется только отказом после вылазки (§12.215): дорога
/// остаётся одна, и та через «нельзя».
///
/// Ворот у склада нет **никаких** — ни науки, ни находки, ни постройки: он
/// стоит в том же ряду, что пол и гараж (`never_locks_its_way_back`), только
/// по другому доводу — не «иначе партия заперта», а «иначе урок теряет половину».
#[test]
fn the_shipped_ruleset_opens_storage_from_the_start() {
    let mut sim = Sim::new(include_str!("../../assets/rulesets/core.yaml")).expect("рулсет");
    let storage = sim.tile_index("storage").expect("склад в палитре");
    assert!(
        sim.tile_is_open(storage as usize),
        "склад закрыт воротами: до первой вылазки его не построить",
    );
}

/// **Боевой рулсет: лежанка открыта с нулевого тика** (§12.223).
///
/// Три её клетки стоят в стартовой застройке, и коты спят в них ещё до первого
/// клика. Ворота по находке (а до §12.223 они у неё были) не открывали ничего —
/// они объявляли новостью постройку, которая всё это время была на экране.
///
/// Довод соседний со складом (`opens_storage_from_the_start`), но не тот же: у
/// склада «иначе теряется половина урока», здесь — «иначе игра сообщает как о
/// новом о том, что игрок уже видел» (§12.120, §12.221).
#[test]
fn the_shipped_ruleset_opens_the_bed_it_already_built() {
    let mut sim = Sim::new(include_str!("../../assets/rulesets/core.yaml")).expect("рулсет");
    let bed = sim.tile_index("bed").expect("лежанка в палитре");
    assert!(
        sim.tile_is_open(bed as usize),
        "лежанка закрыта воротами, а стартовая застройка её уже содержит",
    );
}

/// **Боевой рулсет: первая вылазка не открывает построек, а учёный — ровно
/// одну** (§12.223, §12.260).
///
/// Стопка тикеров гаснет по тикам и не выстраивается в очередь (§12.120):
/// сколько новостей пришло разом, столько игрок и читает сразу. После первой
/// вылазки их было четыре — образец, лаборатория, лежанка, торговый пост, — и
/// три из них ответа не требовали: лежанка стояла на карте с нулевого тика, а
/// постом до постройки склада всё равно нечем было воспользоваться (§12.215,
/// §12.69).
///
/// Остаться должна одна, и та — лаборатория. С §12.260 её приводит учёный, а
/// не находка: пришёл тот, кто умеет спрашивать «что это», — и ему названо
/// ровно одно дело. Ошибка тут
/// тихая — новые ворота у тайла законны, дерево связно, ни один другой сторож
/// не шелохнётся, — а начало партии снова расползается в список.
#[test]
fn the_shipped_ruleset_opens_one_building_by_the_first_find() {
    let mut sim = Sim::new(include_str!("../../assets/rulesets/core.yaml")).expect("рулсет");
    let before: Vec<bool> = (0..sim.palette.len())
        .map(|def| sim.tile_is_open(def))
        .collect();

    sim.sight("sample"); // ровно то, что привозит вылазка с прибором
    let found: Vec<usize> = (0..sim.palette.len())
        .filter(|&def| sim.tile_is_open(def) && !before[def])
        .collect();
    assert!(found.is_empty(), "находка открыла постройки: {found:?}");

    sim.master("science", 1); // пришёл учёный

    let fresh: Vec<usize> = (0..sim.palette.len())
        .filter(|&def| sim.tile_is_open(def) && !before[def])
        .collect();
    let opened: Vec<&str> = fresh
        .iter()
        .map(|&def| sim.palette[def].id.as_str())
        .collect();
    assert_eq!(
        opened,
        ["lab"],
        "учёный открывает {opened:?} — стопка тикеров снова список, \
         а не одно названное дело",
    );
}

/// **Боевой рулсет: цепочка палитры идёт вперёд.** Предшественник ниже по
/// списку не разрешится вовсе — тайл просто откроется сразу, и замысел цепочки
/// развалится молча (§12.210).
#[test]
fn the_shipped_ruleset_chains_its_palette_forward() {
    let sim = Sim::new(include_str!("../../assets/rulesets/core.yaml")).expect("рулсет");
    for (def, prev) in sim.tile_order().iter().enumerate() {
        let Some(prev) = *prev else { continue };
        assert!(
            (prev as usize) < def,
            "тайл {def} ждёт тайл {prev}, стоящий ниже",
        );
    }
}
