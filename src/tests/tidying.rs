...
/// не увидят.
///
/// Под рамку намеренно попадают **две разные цены**: одиннадцать полов по лому и
/// сам гараж за «2 лома + деталь» (§12.152 оставила его одной клеткой). Цена из
/// одного предмета не поймала бы возврат набором.
#[test]
fn the_shipped_ruleset_tidies_demolition_scrap() {
    ...
    assert!(
        ...
        "сносим половину гаражного зала"
    );
    sim.tick_n(3000);
    ...
    assert_eq!(
        sim.scrap_total(),
        before + 14,
        "11 полов по лому и гараж за «2 лома + деталь» вернулись целиком"
    );
