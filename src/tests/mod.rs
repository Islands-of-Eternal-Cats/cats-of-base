...
fn sim_from(rows: &[&str]) -> Sim {
    ...
    // Наборов у ящиков нет: пустой список — это «все клетки принимают всё»
    // (§12.195), то есть поведение до ящиков. Набор заводит `set_bin`.
    ...
    Sim {
        ...
        palette: vec![TileDef {
            ...
            cost: BTreeMap::new(),
            capacity: 0,
            // Ярусов хранения у схемы нет (§12.195): приоритет у всех клеток
            // нулевой, значит уборка ведёт себя ровно как до ящиков — везёт в
            // ближайшее свободное место. Ящик заводит `set_priority`.
            priority: 0,
            rest: 0,
            wake: 0,
            heal: 0,
...
impl Sim {
    ...
    /// Ярус хранения тайла (§12.195). Ноль — обычный склад; выше — ящик, к
    /// которому коты подтаскивают со складов ниже.
    fn set_priority(&mut self, tile: i16, priority: i32) {
        self.tile_rule(tile, |r| r.priority = priority);
    }

    /// Все хранилища палитры парами `(ярус, ёмкость)` — для контентных проверок
    /// (§12.195): ярус и ёмкость обязаны идти в противоход, иначе близость
    /// перестаёт стоить плотности и нижний ярус становится мёртвым контентом.
    fn storage_tiers(&self) -> Vec<(i32, i32)> {
        let rules = self.world.resource::<TileRules>();
        (0..rules.0.len())
            .map(|i| i as i16)
            .filter(|&t| rules.capacity_of(t) > 0)
            .map(|t| (rules.priority_of(t), rules.capacity_of(t)))
            .collect()
    }

    /// Что велено держать в клетке; пусто — принимает всё (§12.195).
    fn bin_of(&self, x: i32, y: i32) -> Vec<usize> {
        self.world
            .resource::<Bins>()
            .of(x, y)
    ...
    /// Сколько ячеек лаборатории стоит на карте — для контентных проверок
    /// (§12.132): ячейка это слот темы, и комната, вымощенная ею целиком,
    /// молча выключает счётность (§12.55).
    fn lab_cells(&self) -> usize {
        let map = self.world.resource::<BaseMap>();
        let rules = self.world.resource::<TileRules>();
        (0..map.height)
            .flat_map(|y| (0..map.width).map(move |x| (x, y)))
            .filter(|&(x, y)| rules.is_lab(map.tile_at(x, y)))
            .count()
    }
    ...
    /// Улучшение (§12.238): `tile` ставится только поверх `below` и стирается
    /// обратно до него.
    fn set_on(&mut self, tile: i16, below: i16) {
        self.tile_rule(tile, |r| r.on = Some(below));
    }
    ...
    /// Знакома ли база с фракцией — **тем же выражением, каким это считает
    /// шапка** (§12.214): снимок на хосте не собрать, а второй экземпляр
    /// правила показал бы игроку не то, что проверено (инвариант 14).
    fn faction_is_met(&self, faction: usize) -> bool {
        crate::missions::faction_is_met(
            self.world.resource::<MissionRules>(),
            self.world.resource::<Raids>(),
            faction,
        )
    }
    ...
    ///
    /// Со §12.208 стартовая застройка их не содержит — парту и лабораторию
    ...
    /// Сделать тайл рацией: на такой клетке дежурят, и она даёт отряду в поле
    /// прибавку к силе (§12.60).
    ///
    /// **Слота вылазки она не даёт** с §12.152 — его держит гараж (`set_gate`),
    /// — поэтому тестам, которым нужна только вылазка, рация не нужна вовсе.
    /// В схеме `sim_from` раций нет, как нет ни шлюза, ни мастерской; а сила
    /// связи у них ноль (`set_comms`), то есть дежурить незачем и раздатчик
    /// никого к ним не зовёт.
    ...
    /// Постоянный состав отряда этого гаража — ровно то, что возьмёт
    /// `launch_node` и что увидит в панели игрок (§12.61, §12.152).
    ...
    /// Сколько материала завезли на станок единственного заказа (§12.102).
    /// Сумма по типам: это полоска наполнения, а не запас, — ровно то же число,
    /// что панель пишет как «завезено N из M».
    fn craft_delivered(&mut self) -> Option<i32> {
        let mut q = self.world.query::<&Craft>();
        q.iter(&self.world)
            .next()
    ...
    fn mission_gate(&mut self) -> Option<(i32, i32)> {
        let mut q = self.world.query::<&Mission>();
        q.iter(&self.world).next().map(|m| m.gate)
    }

    /// Гараж, который держит слот вылазки по этому заказу (§12.61, §12.152).
    /// Адресуется по заказу, а не по номеру: порядок обхода ECS недетерминирован.
    fn mission_node(&mut self, def: usize) -> Option<(i32, i32)> {
        let mut q = self.world.query::<&Mission>();
        q.iter(&self.world).find(|m| m.def == def).map(|m| m.gate)
    }
    ...
    /// Почему правило узла стоит (§12.116, §12.117) — ровно теми выражениями,
    /// какими это решает `run_auto_raids`: `(доля, провал, в сборе ли отряд)`.
    /// Снапшот на хосте не собрать, а панель говорит именно это.
    fn auto_hold_at(&mut self, x: i32, y: i32) -> (i32, bool, bool) {
