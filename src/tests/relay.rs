...
/// Отряд ушёл, рация свободна — и **никто не садится сам** (§12.76). Дежурство
...
/// В `core.yaml` **построенная** рация работает: за ней сидят и она прибавляет
/// отряду силы. Ловит рулсет, где тайл завели, а `comms` забыли, — то есть весь
/// §12.60 молча выключен.
///
/// Строить её приходится прямо здесь: с §12.152 в стартовой застройке рации нет
/// (слота она не держит, и первая вылазка уходит без связи). Это и есть та
/// разница, которую игрок открывает сам, — а тест повторяет его путь.
#[test]
fn the_shipped_ruleset_puts_someone_on_the_radio() {
    ...
    assert_eq!(sim.relay_count(), 0, "на старте её нет");

    // Ставим рацию в гаражном зале — там, где она стояла до §12.152.
    let node = (3, 10);
    sim.force_tile(node.0, node.1, relay);

    // Сам по себе к рации никто не садится (§12.76) — связиста назначает игрок.
