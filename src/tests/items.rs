...
/// Площадка, которой нужны два типа, ждёт оба: пока детали нет, стройка не
/// начинается, даже если лома с запасом, — и лом на неё не везут, чтобы не
/// запереть его в площадке, которую нечем закрыть (§12.262).
#[test]
