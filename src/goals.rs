...
pub(crate) fn check_goals(
    ...
    // Очередь читается по состоянию **на начало тика** (§12.209): цель, чей
    // предшественник взят прямо сейчас, ждёт следующего тика. Без этого среза
    // цепочка целей с уже выполненными условиями схлопнулась бы в один тик — то
    // есть игрок увидел бы разом весь список уже взятым, ровно то, ради отмены
    // чего очередь и заведена.
    let opened: Vec<bool> = (0..rules.0.len())
        .map(|def| goals.taken(def).is_some())
        .collect();
    for (def, rule) in rules.0.iter().enumerate() {
        ...
        // Цели, до которой очередь не дошла, **не существует** (§12.209): её не
        // видно и её нельзя взять. Второе не строгость ради строгости — «100
        // лома на складе» набирается раньше первой вылазки, и без этой строки
        // цель всплыла бы уже взятой, то есть игрок увидел бы галочку вместо
        // задачи. Отметок она тоже не ведёт: дата условия у цели, которой ещё
        // нет, — это дата не о ней (§12.159).
        if rule.after.is_some_and(|prev| !opened[prev]) {
            continue;
        }
