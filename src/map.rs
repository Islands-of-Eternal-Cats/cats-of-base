...
pub(crate) struct BaseMap {
    ...
    /// **Завал по клеткам: на сколько тиков шаг в клетку длиннее чистого**
    /// (§12.35, §12.249). Считается из куч в начале каждого тика
    /// (`movement::note_clutter`), в снимок не идёт (выводится из куч) и
    /// **версию не двигает**: версия говорит «маршруты могли стать
    /// невозможными», а завал меняет только их цену. Живёт в карте, а не своим
    /// ресурсом, потому что читают его двое — шаг (`move_units`) и поиск пути
    /// (`Reach`), — и оба уже держат `&BaseMap`; вторая выборка куч у одного
    /// из них разошлась бы с первой на тик.
    pub(crate) drag: Vec<u8>,
}

/// Чистый темп шага в тиках: тик, в который кот переставляет лапы, плюс период
/// остывания (`MOVE_PERIOD` = 1). При `BASE_TPS = 6` — три клетки в секунду.
pub(crate) const STEP_TICKS: u8 = 2;
...
impl BaseMap {
    pub(crate) fn empty(width: i32, height: i32) -> Self {
        BaseMap {
            ...
            version: 0,
            ever: Vec::new(),
            drag: vec![0; (width * height) as usize],
        }
    }

    /// Сколько тиков занимает шаг **в** клетку: чистый темп плюс завал в ней
    /// (§12.35). **Одно выражение на шаг и на поиск пути** (§12.249): по нему
    /// `move_units` растягивает шаг, и по нему же `Reach` считает, какой из
    /// маршрутов короче по времени. Разойдись они — кот выбирал бы обход,
    /// который на деле дольше прямого.
    pub(crate) fn step_cost(&self, x: i32, y: i32) -> u8 {
        STEP_TICKS + self.index(x, y).map_or(0, |i| self.drag[i])
    }

    /// Переписать завал целиком: `cells` — пары (клетка, задержка), прочие
    /// клетки чисты. Версию не трогает намеренно (см. `drag`).
    pub(crate) fn set_drag(&mut self, cells: impl IntoIterator<Item = ((i32, i32), u8)>) {
