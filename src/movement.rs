...
use crate::map::{BaseMap, DIRS, STEP_TICKS};
use crate::path::{Reach, find_path};

/// Сколько штук в куче стоит одного лишнего тика на шаг (§12.35).
///
/// С §12.248 ставка вдвое круче прежней (была 8): чистый шаг — два тика, и
/// полный склад (20 штук) стоит теперь +5, то есть кот бредёт через него в
/// три с половиной раза дольше, чем по полу. На ×10 удвоение шага глаз не
/// читал вовсе — «коты стали медленнее», а не «здесь вязко»; разница втрое и
/// больше видна как препятствие на любой скорости.
const CLUTTER_PER_TICK: i32 = 4;
...
/// дольше шаг **в неё**, — до потолка. **Считается всё, что лежит в клетке,
/// куда кот ступает, — и на полу, и на складе** (§12.248): проходимое
/// хранилище с §12.248 — это пол, по которому ходят между сложенным, и полный
/// склад тормозит, как та же куча в коридоре. До §12.248 склад из счёта
/// исключался («сложенное — порядок, а не завал»), и оттого проходимая клетка
/// с ёмкостью была полом без единственного недостатка пола: оптимальная база
/// вырождалась в сплошной квадрат складов. Быстрое хранение теперь ровно одно —
/// непроходимое (`solid`, §12.142): на стеллаж не ступают, значит, он не
/// тормозит никого, а проходы к нему требует правило доступа (§12.111).
/// До §12.140 это была пауза **после** шага, и в игре она читалась дёрганьем;
/// теперь это цена входа, и кот через кучу бредёт.
///
/// **Маршрут о завале знает** (§12.249, отменяет §11 и §12.35 в этой части):
/// цена шага берётся из карты (`BaseMap::step_cost`), и по тому же выражению
/// `Reach` ищет кратчайший **по времени** путь. Кот обходит кучи по дороге и
/// идёт напрямик к той, за которой послан. Завал сюда кладёт `note_clutter`
/// в начале тика.
pub(crate) fn move_units(
    ...
    for (e, mut pos, mut path, stride) in &mut q {
        ...
        let span = map.step_cost(next.0, next.1);
...
/// Переписывает завал по клеткам в карту (§12.249): сколько лишних тиков
/// стоит шаг **в** клетку с кучей. Стоит **первой в цепочке**: раздатчики
/// меряют «кто ближе» через `Reach`, а тот берёт цену шага из карты, — и с
/// завалом прошлого тика раздача разошлась бы с ходом на тик.
///
/// Считается всё, что лежит в клетке, — и на полу, и на складе (§12.248):
/// проходимое хранилище — это пол, по которому ходят между сложенным, и
/// полный склад тормозит, как та же куча в коридоре. До §12.248 склад из
/// счёта исключался («сложенное — порядок, а не завал»), и оттого проходимая
/// клетка с ёмкостью была полом без единственного недостатка пола:
/// оптимальная база вырождалась в сплошной квадрат складов. Быстрое хранение
/// ровно одно — непроходимое (`solid`, §12.142): на стеллаж не ступают,
/// значит, он не тормозит никого, а проходы к нему требует правило доступа
/// (§12.111).
///
/// Задержка ложится на клетку, **в которую** кот шагает: он пробирается через
/// кучу. Так замедление видно там же, где видна куча.
pub(crate) fn note_clutter(mut map: ResMut<BaseMap>, stacks: Query<(&Position, &Stack)>) {
    ...
    map.set_drag(clutter.into_iter().map(|(at, n)| {
        (
            at,
            (n / CLUTTER_PER_TICK).clamp(0, CLUTTER_MAX as i32) as u8,
        )
    }));
}

/// На сколько тиков шаг длиннее чистого — то, что в нём от завала (§12.248).
///
/// Считает ядро, а не вид: чистый темп — константа карты, и вычитать его в JS
/// значило бы завести там второй экземпляр `step_cost`.
pub(crate) fn step_drag(span: u8) -> u8 {
    span.saturating_sub(STEP_TICKS)
}
