...
pub(crate) fn move_units(
    ...
    let clutter = clutter_map(&stacks);
...
/// Что и сколько лежит в каждой клетке — на полу и на складе (§12.248).
fn clutter_map(
    stacks: &Query<(&Position, &Stack), Without<Path>>,
) -> Vec<((i32, i32), i32)> {
...
///
/// Считает ядро, а не вид: чистый темп — константа этого модуля, и вычитать
/// его в JS значило бы завести там второй экземпляр `step_span`.
pub(crate) fn step_drag(span: u8) -> u8 {
    span.saturating_sub(MOVE_PERIOD + 1)
}
