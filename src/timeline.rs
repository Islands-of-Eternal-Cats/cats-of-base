...
//!
//! С §12.198 у события есть и **адрес**: `seeds:` сажает очаг на участок карты.
//! Это тот же «мир действует сам», только вторым концом — расписание называет
//! дату, карта распространяет, — и вместе они делают записку (§4.6) картой
//! путей, а не списком дат.
//!
...
pub(crate) fn run_timeline(
    ...
    for (def, rule) in rules.0.iter().enumerate() {
        ...
        // Посев на карте (§12.198) — **до развилки по готовности и независимо
        // от неё**. Очаг не подарок и не штраф за неуспетую технологию: это
        // событие мира, на которое базе ещё предстоит ответить, — и отвечает
        // она вылазкой, а не тем, что успела изучить. Здесь женятся два
        // источника «мир действует сам»: расписание называет дату, карта —
        // адрес, и записка (§4.6) перестаёт быть списком абстрактных бедствий.
        for &(site, blight) in &rule.seeds {
            blights.seed(site, blight);
        }
