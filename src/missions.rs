...
pub(crate) fn phase(rule: &MissionRule, span: i32, left: i32) -> Phase {
    // Работа — это всё, что срок набрал сверх дороги: `duration` складывает его
    // ровно так, и вычитание здесь не «оценка», а тот же член суммы.
    let work = (span - rule.travel).max(0);
    let there = rule.travel / 2;
    let gone = span - left;
    if gone < there {
        Phase::Travel
    } else if gone < there + work {
        Phase::Work
    } else {
        Phase::Back
    }
